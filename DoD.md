...
- [x] [ARTIFACT] `scripts/product-map/cli.mjs` 实现 `check --json`，测试文件保留真实子进程覆盖
...
- [x] [BEHAVIOR] [L2] B-01: Golden Path 成功时调用方读到精确 JSON schema
...
- [x] [BEHAVIOR] [L2] B-02: 漂移失败时仍返回具体 errors 与非零退出
...
- [x] [BEHAVIOR] [L2] B-03: 投影缺失或损坏不泄漏非 JSON stdout
...
- [x] [BEHAVIOR] [L2] B-06: 产品分类合同原入口继续通过
  Test: manual:bash -c 'npm run product-map:check'

- [x] [BEHAVIOR] [L2] B-07: JSON 标志与既有参数并存时互不干扰

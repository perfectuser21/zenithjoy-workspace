...
动任何东西之前——**包括只改设备设置**——先拿到现场：**前台包名 + agent 诊断行 + 截图**。
缺一件 = 你在猜；猜着改设备或改代码，一律停下先去抓。

> 0821 私信排查实证：据"同源"猜着禁了两个荣耀 ROM 组件、白改三轮，读到 logcat 才发现
> `searchBtnFound=true`、真凶是点完之后前台被抢走——先前的判断被现场直接推翻。
> 完整依据与机械闸见 invariant `93ed0761`（CI: `lint-rpa-failure-scene`）。

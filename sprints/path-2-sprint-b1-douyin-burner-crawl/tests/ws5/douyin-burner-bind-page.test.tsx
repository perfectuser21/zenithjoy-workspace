/**
 * WS5 — DouyinBurnerBindPage.tsx (dashboard 绑小号 + 抓评论页)
 */
import { describe, it, expect, beforeEach, vi } from 'vitest';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import { MemoryRouter } from 'react-router-dom';
import DouyinBurnerBindPage from '../../../../apps/dashboard/src/pages/DouyinBurnerBindPage';
...
describe('Workstream 5 — DouyinBurnerBindPage [BEHAVIOR]', () => {
  ...
  it('抓取完成（status=done + comment_count=5）→ 显示「抓取完成 5 条」+ Bitable URL', async () => {
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ success: true, data: { sessions: [{ account_label: 'A', role: 'burner', status: 'active', account_nickname: 'A' }] } }),
    } as any);
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({

/**
 * WS6 — smoke 脚本结构 + fake-feishu-server seen-records 增量
 */
import { describe, it, expect } from 'vitest';
import * as fs from 'fs';
import * as path from 'path';

const SMOKE = path.resolve(__dirname, '../../../../.github/workflows/scripts/smoke/golden-path-2-b1-smoke.sh');
const FAKE_FEISHU = path.resolve(__dirname, '../../../../apps/api/test-utils/fake-feishu-server.ts');

describe('Workstream 6 — smoke 脚本结构 [BEHAVIOR]', () => {
  it('smoke 脚本存在 + 可执行', () => {
    expect(fs.existsSync(SMOKE)).toBe(true);
    const stat = fs.statSync(SMOKE);
    // mode & 0o111 检查 owner+group+other 至少一个 x bit

...
TEN_A=$(psql "$DB" -t -A -c "INSERT INTO zenithjoy.tenants(name,license_key) VALUES ('e2e-cw-a','e2e-cw-license-a') RETURNING id" | head -1 | tr -d '[:space:]')
TEN_B=$(psql "$DB" -t -A -c "INSERT INTO zenithjoy.tenants(name,license_key) VALUES ('e2e-cw-b','e2e-cw-license-b') RETURNING id" | head -1 | tr -d '[:space:]')
TEN_C=$(psql "$DB" -t -A -c "INSERT INTO zenithjoy.tenants(name,license_key) VALUES ('e2e-cw-c','e2e-cw-license-c') RETURNING id" | head -1 | tr -d '[:space:]')
TEN_D=$(psql "$DB" -t -A -c "INSERT INTO zenithjoy.tenants(name,license_key) VALUES ('e2e-cw-d','e2e-cw-license-d') RETURNING id" | head -1 | tr -d '[:space:]')
...
# CI 环境每次新建租户（不同 license_key），service_agents 无旧行，直接 INSERT。
psql "$DB" -q -c "

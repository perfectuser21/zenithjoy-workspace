...
- [ ] [ARTIFACT] publish-douyin-image.cjs 改用 qr-login（铁律 6：先减肥）
  Test: grep -E "require.*lib/qr-login" services/agent/publishers/douyin-publisher/publish-douyin-image.cjs

- [ ] [ARTIFACT] 旧 image.cjs 中"假设已登录直接发"代码已删
  Test: ! grep -iE "(skip.*login|assume.*logged.*in|已登录)" services/agent/publishers/douyin-publisher/publish-douyin-image.cjs

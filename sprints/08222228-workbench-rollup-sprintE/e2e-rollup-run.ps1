# 路③ Sprint E —— windows_cloud rollup 链 E2E：一个 step 一次自持全跑
#
# 用法（workflow 里三个 step 各调一次，标签 ASCII 防 Windows grep 解码歪成 "No tests found"）：
#   pwsh -File e2e-rollup-run.ps1 -Grep "@rollup-build"
#   pwsh -File e2e-rollup-run.ps1 -Grep "@rollup-lookup"
#   pwsh -File e2e-rollup-run.ps1 -Grep "@rollup-degrade"
#
...
param(
  ...
  [string]$Spec = "structured-workbench-rollup.spec.ts"
)
...
$ApiPort = if ($env:PATH3_ROLLUP_API_PORT) { [int]$env:PATH3_ROLLUP_API_PORT } else { 5221 }
$HubPort = if ($env:PATH3_ROLLUP_HUB_PORT) { [int]$env:PATH3_ROLLUP_HUB_PORT } else { 5179 }
...
try {
  ...
  # 甲(alice 表主) 一份真 cookie 即可覆盖 rollup 链（rollup 是本 org 内聚合，无需第二身份）
  $env:E2E_ROLLUP_COOKIE = Get-SessionCookie $ApiPort $seed.Alice
  ...
  Invoke-Checked "npx.cmd" "playwright test $Spec --grep $Grep --reporter=list" `
    "$repoRoot\apps\staff-hub" "staff-hub 路③ S4 rollup 链 E2E ($Spec / $Grep)"
} finally {
  Stop-Procs @($api, $hub)
}
...
# "The property 'Count' cannot be found"（@rollup-build 只产一张图正好踩这个坑）。
...
Write-Host "windows_cloud 路③ S4 rollup 链 E2E 通过（$Grep，本轮新截图 $($fresh.Count) 张）"
exit 0

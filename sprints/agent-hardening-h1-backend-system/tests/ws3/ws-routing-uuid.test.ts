...
const ROOT = join(__dirname, '..', '..', '..', '..', 'apps', 'api', 'src', 'services');
...
describe('WS3 — WS routing UUID 化 [BEHAVIOR]', () => {
  ...
  it('agent-protocol.ts publish_request payload 应可含 agent_id 字段', () => {
    const c = readFileSync(join(__dirname, '..', '..', '..', '..', 'apps', 'api', 'src', 'schemas', 'agent-protocol.ts'), 'utf8');
    expect(c).toMatch(/agent_id/);
  });
  ...
  it('findOrCreateAgentUuid 输入 string display 返 UUID — 单元 mock 验', async () => {
    ...
    let mod: any;
    try {
      mod = await import('../../../../apps/api/src/services/agent-db');
    } catch (err) {

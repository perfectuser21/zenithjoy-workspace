...
describe('preview 不需要 Meta 凭据（但发布路径的闸一个不能少）', () => {
  /** 只配了本端点的 API key，Meta 凭据全缺——模拟凭据到位前的真实状态。 */
  ...
  it('凭据未配置时 preview 仍可用，且不触碰 Graph API', async () => {
    ...
    expect(fetchSpy).not.toHaveBeenCalled();   // 绝不外发
    vi.unstubAllGlobals();
  });
  ...
  it('preview 响应不回显任何密钥', async () => {
    const res = await publishPost({
      ...
      env: env({ META_IDEMPOTENCY: memStore() }),
    } as any);

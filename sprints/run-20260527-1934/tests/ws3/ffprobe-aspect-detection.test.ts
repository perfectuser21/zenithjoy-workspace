...
describe('WS3 — ffprobe width/height + detectedAspect + 比例选择 [BEHAVIOR]', () => {
  it('detectAspect 函数在 video-pipeline.ts 中定义且导出', () => {
    const src = readFileSync(WORKER, 'utf8');
    // WS3 实现前: 无 detectAspect → FAIL
    expect(src).toContain('detectAspect');
    // 必须导出，以便 tsx 直接调用执行验证
    expect(src).toMatch(/export.*detectAspect|detectAspect.*export/);
  });
  ...
  it('migration 含 detected_aspect + target_aspect 列定义', () => {
    ...
    expect(foundDetected).toBe(true);
  });

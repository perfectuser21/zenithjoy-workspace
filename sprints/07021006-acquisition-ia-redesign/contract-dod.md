...
- [ ] [ARTIFACT] `apps/dashboard/src/pages/acquisition/AcquisitionHubPage.tsx`（或原 `AcquisitionHubPage.tsx`）已改为4模块卡片布局，含 `data-testid=hub-card-accounts/tasks/analytics/outreach` 4个卡片节点
  Test: node -e "const {existsSync,readFileSync}=require('fs');const paths=['apps/dashboard/src/pages/acquisition/AcquisitionHubPage.tsx','apps/dashboard/src/pages/AcquisitionHubPage.tsx'];const p=paths.find(x=>existsSync(x));if(!p){console.error('FAIL: AcquisitionHubPage.tsx 不存在（两个路径均未找到）');process.exit(1);}const c=readFileSync(p,'utf8');if(!c.includes('hub-card-accounts')||!c.includes('hub-card-tasks')){console.error('FAIL: 缺少 hub-card testid');process.exit(1);}console.log('✅ hub cards ok path='+p)"
...
- [ ] [BEHAVIOR] collect-tasks/:id/videos 字段集完整性：data.videos 元素含 video_id/task_id/title/thumbnail_url/publish_date/comment_count（空 videos=[] 时跳过字段检查）；禁用字段 videoList/video_list/items/results 不出现
  Test: manual:bash -c 'TASK_ID=$(curl -sf -X POST -H "Content-Type: application/json" -H "X-Tenant-Id: e2e-tenant-test-001" -d '"'"'{"keywords":["field-check"]}'"'"' localhost:3000/api/acquisition/collect/start | jq -r '"'"'.data.task_id'"'"'); RESP=$(curl -sf -H "X-Tenant-Id: e2e-tenant-test-001" "localhost:3000/api/acquisition/collect-tasks/$TASK_ID/videos"); LEN=$(echo "$RESP" | jq '"'"'.data.videos | length'"'"'); if [ "$LEN" -gt 0 ]; then echo "$RESP" | jq -e '"'"'.data.videos[0] | has("video_id") and has("task_id") and has("title") and has("thumbnail_url") and has("publish_date") and has("comment_count")'"'"' || exit 1; fi; echo "$RESP" | jq -e '"'"'.data | (has("videoList") | not) and (has("video_list") | not) and (has("items") | not) and (has("results") | not)'"'"' || { echo "FAIL: 禁用字段"; exit 1; }; echo OK'
  期望: OK（exit 0）
...
- [ ] [BEHAVIOR] GET /api/acquisition/videos/:videoId/leads 新端点 — seed 已知 video_id + 1条 lead，验 200 路径 schema（leads:array, total:number）+ lead entry 字段（commenter_id/comment_text/source_video_url/grade/profile_url）+ 禁用字段 comments/items/results 均不出现
  Test: manual:bash -c 'TASK_ID=$(curl -sf -X POST -H "Content-Type: application/json" -H "X-Tenant-Id: e2e-tenant-test-001" -d '"'"'{"keywords":["leads-schema-test"]}'"'"' localhost:3000/api/acquisition/collect/start | jq -r '"'"'.data.task_id'"'"'); VID="e2e-video-dod-schema"; psql "$DATABASE_URL" -c "INSERT INTO zenithjoy.acquisition_collect_videos (video_id,task_id,tenant_id,title,thumbnail_url,publish_date,comment_count) VALUES ('"'"'$VID'"'"','"'"'$TASK_ID'"'"','"'"'e2e-tenant-test-001'"'"','"'"'DOD Test'"'"',NULL,NULL,1) ON CONFLICT (video_id) DO UPDATE SET task_id='"'"'$TASK_ID'"'"',tenant_id='"'"'e2e-tenant-test-001'"'"',comment_count=1"; psql "$DATABASE_URL" -c "INSERT INTO zenithjoy.leads (commenter_id,comment_text,source_video_url,source_video_id,tenant_id,keyword,crawled_at) VALUES ('"'"'dod-uid-001'"'"','"'"'DOD test comment'"'"','"'"'https://v.douyin.com/'"'"'||'"'"'$VID'"'"','"'"'$VID'"'"','"'"'e2e-tenant-test-001'"'"','"'"'leads-schema-test'"'"',NOW()) ON CONFLICT DO NOTHING" 2>/dev/null || true; RESP=$(curl -sf -H "X-Tenant-Id: e2e-tenant-test-001" "localhost:3000/api/acquisition/videos/$VID/leads"); echo "$RESP" | jq -e '"'"'.success == true'"'"' || exit 1; echo "$RESP" | jq -e '"'"'.data.leads | type == "array"'"'"' || exit 1; echo "$RESP" | jq -e '"'"'.data.total | type == "number"'"'"' || exit 1; LEN=$(echo "$RESP" | jq '"'"'.data.leads | length'"'"'); if [ "$LEN" -gt 0 ]; then echo "$RESP" | jq -e '"'"'.data.leads[0] | has("commenter_id") and has("comment_text") and has("source_video_url") and has("grade") and has("profile_url")'"'"' || { echo "FAIL: lead entry 缺少必填字段（commenter_id/comment_text/source_video_url/grade/profile_url）"; exit 1; }; fi; echo "$RESP" | jq -e '"'"'.data | (has("comments") | not) and (has("items") | not) and (has("results") | not)'"'"' || { echo "FAIL: 禁用字段"; exit 1; }; echo OK'
  期望: OK（exit 0）

...
```bash
DATABASE_URL="${DATABASE_URL:?}" npx vitest run sprints/0802135715-ab-kernel-acquisition-config-amended-runner/tests/acquisition-config-effective-validation.integration.test.ts -t "合法部分更新成功持久化并可读取且不改变另一租户"
```
...
```bash
DATABASE_URL="${DATABASE_URL:?}" npx vitest run sprints/0802135715-ab-kernel-acquisition-config-amended-runner/tests/acquisition-config-effective-validation.integration.test.ts -t "有效配置 min>max 返回 400 INVALID_CONFIG"
```
...
```bash
DATABASE_URL="${DATABASE_URL:?}" npx vitest run sprints/0802135715-ab-kernel-acquisition-config-amended-runner/tests/acquisition-config-effective-validation.integration.test.ts
```
...
```bash
rg -n "vi\.mock|jest\.mock|page\.route|stub" sprints/0802135715-ab-kernel-acquisition-config-amended-runner/tests/acquisition-config-effective-validation.integration.test.ts && exit 1 || test $? -eq 1
```
...
TEST_FILE="sprints/0802135715-ab-kernel-acquisition-config-amended-runner/tests/acquisition-config-effective-validation.integration.test.ts"
START=$(date +%s)

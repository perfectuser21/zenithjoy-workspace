...
- **对应 KR**：KR-未提供（Cecelia 基础稳固 — 系统可信赖、算力全开、管家闭环）
- **当前进度**：82%
- **本次推进预期**：使 product-map 检查结果可被 CI 与 Brain 稳定解析

## 背景

`scripts/product-map/cli.mjs` 的 `check` 子命令目前只打印人类可读文本，CI 与 Brain 只能解析易漂移的 stdout。此次增加 `--json` 输出模式，同时保持现有文本输出和退出码兼容。

## Golden Path（核心场景）

运维/CI 执行 `node scripts/product-map/cli.mjs check --json` → 命令输出单个合法 JSON 对象 → 调用方用 `jq` 读取 `ok` 与 `errors` 并结合退出码完成门禁判定。

具体：
1. 调用方执行 product-map CLI 的 `check --json`。
2. 检查通过时，stdout 仅输出一个 JSON 对象，`ok` 为 `true`、`errors` 为空数组，退出码为 0。
3. 检查失败时，stdout 仍仅输出一个合法 JSON 对象，`ok` 为 `false`、`errors` 包含具体问题，退出码非 0。
4. 不带 `--json` 时，输出与当前版本逐字一致，退出码语义不变。

## 边界情况

- `product-map.json` 不存在或不可解析时，`--json` 仍输出合法 JSON，且 `errors` 包含原因，不向 stdout 泄漏未捕获异常。
- `--json` 与既有参数并存时不改变其他参数语义。
- 多个检查问题同时存在时，`errors` 为字符串数组并逐项表达具体问题。
- 不带 `--json` 的成功与失败路径均保持现有文本格式。

## 范围限定

**在范围内**：`scripts/product-map/` 下 CLI 的 `check --json` 行为及其单元测试。
**不在范围内**：修改 product-map 数据、改变其他子命令、改变无 `--json` 时的输出、改变既有退出码语义。

## 假设

- [ASSUMPTION: Brain 上下文未提供 KR 编号，以当前活跃且最相关的“Cecelia 基础稳固”KR 标题及 82% 进度对齐。]
- [ASSUMPTION: `errors` 的具体文案沿用现有检查问题文本，不在 Planner 阶段锁定内部错误分类。]
- [ASSUMPTION: `--json` 只改变 stdout 表达形式，不新增文件输出或网络调用。]

## 预期受影响文件

- `scripts/product-map/cli.mjs`：承载 product-map CLI `check --json` 的用户可见行为。
- `scripts/product-map/` 下对应单元测试：覆盖 JSON 成功、JSON 失败、异常输入及文本模式零回归。
...
- 频控: 不适用（本地 CLI）
- 版本要求: 待定（PrepPRD 未指定）
- 可观测: stdout 在 `--json` 模式必须始终是单个可解析 JSON 对象；退出码保持通过 0、失败非 0
...
- [smoke-] smoke 铁律（来源: area）
- [测试如果全部] 测试如果全部依赖"重置状态=冷启动"的写法（`afterEach` 清空 sentinel、传 `sinceMs=0`），要专门补至少一条"真实多轮扫描、状态不重置、时间真实流逝"的集成测试，否则这类"跨扫描周期"的 bug 永远测不出来（来源: area）
- [涉及"周期性] 涉及"周期性重新扫描同一批数据"的设计，一旦引入外部付费调用（LLM/第三方API），必须同时设计"是否已处理过"的前置检查，不能假设"重扫不常发生"就不用防——扩大扫描窗口（为了修一个 bug）反而可能意外放大另一个本来隐藏很浅的问题（来源: area）
- [跨模块的"时] 跨模块的"时间常数"（扫描间隔、闲置阈值、缓存 TTL 等）如果彼此之间有隐含的大小关系依赖，必须在设计阶段显式写一条不变量断言或注释（比如"必须保证 LOOKBACK_WINDOW > IDLE_THRESHOLD"），不能指望测试覆盖到——本次这个 bug 潜伏在 3 个独立 Task 的接缝处，任何单个 Task 的测试都测不出来，只有对整个分支做"跨任务组合"审查的最后一轮才抓到（来源: area）
...
- [DB 表字段] DB 表字段长度约束（如 `varchar(100)`）在写入前若来源数据没有天然长度保证（如文件系统路径/目录名），必须显式截断，不能假设"看起来不会太长"——本次触发条件（嵌套 worktree 路径）就存在于开发者自己的日常工作模式里，不是边缘 case（来源: area）
- [复活/重做一] 复活/重做一个曾经死过的功能前，先用 `git log --diff-filter=D` + `git show <commit>:<path>` 读退役前的真实代码，逐字核对 death cause，不要只信退役 commit message 的一句话总结——本次靠这个方法把"死因不明的历史教训"变成了"可复现、可规避的具体 bug 模式"（来源: area）
- [调用任何"失] 调用任何"失败不抛异常，返回 null/false 表示失败"契约的函数时，写完 `if (成功分支)` 一定要显式写 `else` 处理失败分支，不能只依赖外层 `try/catch`——这类"错误码而非异常"的契约在本仓库很常见（`pushCapture`/`claimDedupeKey` 等），review 时应主动搜索"这个函数会不会抛异常"再判断调用方的错误处理是否对得上（来源: area）
- [smoke-] smoke 铁律（来源: area）
...
- [catch] catch 吞错的后台 job 必须带失败计数指标，连续失败超阈值告警（inbox P1 账龄哨兵将覆盖）（来源: area）
- [表名认领冲突] 表名认领冲突：建新表/复用表前先 grep 全部写入方，两个模块写同一张表必须 schema 对齐评审（来源: area）
- [新增后台 j] 新增后台 job 必须同时声明消费方——无下游读方的落库 job 不允许上线（inbox 统一设计已立为死规矩：每条路由必须有真实消费者）（来源: area）
...
- [同一语义（如] 同一语义（如 git_sha=unknown）在判变端与终验端必须同一处理策略，跨脚本语义分叉会开假绿面（来源: area）
- [`git r] `git rev-parse` 判 ref 存在必须带 `--verify "<ref>^{commit}"`，裸 rev-parse 失败回显字面量（来源: area）
- [smoke/] smoke/测试用真实 worktree 当 CECELIA_DEPLOY_ROOT 时，必须核对被测脚本会不会向上触碰生产资源（brain-deploy、git tag 向上找共享 refs、/tmp 状态文件）——SKIP 钩子逐个显式设，跳过项列在 smoke 头注释（来源: area）
- [部署链任何失] 部署链任何失败路径禁止 warning 降级：显式 FAIL 变量 + Bark + exit 非零（set -uo 无 -e 的脚本尤其注意管道赋值 `|| echo ""` 兜底，grep 空结果 + pipefail 会静默炸死 set -e 脚本）（来源: area）
...
- [smoke-] smoke 铁律（来源: area）
- [feat+b] feat+brain/src PR 开 PR 前直接一次带齐 smoke.sh + smoke-allowlist 登记，别等 CI 两连红（来源: area）
- [新 task] 新 task_type 接线用七点清单：CHECK 约束 / task-router 四表 / EXECUTOR_KIND_FOR / executor dispatch 分支 / executor override 排除 / relay loadSkill 映射 / dispatcher cap+lock+bridge 三防线（来源: area）
- [服务"该活着] 服务"该活着"的判定用双信号：launchctl 状态 + 端口监听（单看 launchd 漏 nohup 孤儿宕机，判定点决策 d172e54a）（来源: area）
- [本机（美国] 本机（美国 Mac mini）**禁止再往 `~/Library/LaunchAgents` 放需要常驻的服务**——gui 域不存在，永不加载；用系统域 LaunchDaemon + `UserName=administrator`（bridge 先例）（来源: area）
- [新增常驻宿主] 新增常驻宿主服务时，必须同步加进 `packages/brain/src/launchd-patrol.js` 的 manifest（MUST_RUN_DAEMONS / MUST_LOAD_DAEMONS / MUST_LISTEN_PORTS）（来源: area）
- [smoke-] smoke 铁律（来源: area）
...
```bash
# 占位：proposer 将按 target_environment 填入真实脚本（local_api→本地 CLI 命令）
# 期望验收点（自然语言）：真实执行 check --json 后，stdout 可由 jq 读取 ok/errors，成功与失败退出码正确；不带 --json 的输出逐字保持兼容。
```

## journey_type: autonomous
## journey_type_reason: 该工作是 scripts/product-map 下的非交互式 CLI 行为，无 UI 或远端 Agent 协议。
## target_environment: local_api
## target_environment_reason: 任务 payload 显式指定 local_api，在本地执行 Node CLI 与 jq 即可完成端到端验收。

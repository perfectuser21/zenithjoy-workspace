...
- [ ] [ARTIFACT] `scripts/product-map/cli.mjs` 实现 JSON 模式且对应正式测试入册
  Test: node -e "const fs=require('fs');const cli=fs.readFileSync('scripts/product-map/cli.mjs','utf8');const reg=fs.readFileSync('test-registry.yaml','utf8');if(!cli.includes('--json')||!reg.includes('product-map-cli-json'))process.exit(1)"
...
- [ ] [BEHAVIOR] [L2] B-05: JSON 标志与既有参数并存
  动作: 执行 `check --json --verbose`
  预期观察: 仍只输出成功 JSON，参数不互相干扰
  等待预算: 5s
  留证: node:test TAP 中 `其他既有参数并存` 子测试输出
  Test: manual:bash -c 'node --test --test-name-pattern="其他既有参数并存" sprints/08061215-productmap-cli-json-r42/tests/product-map-cli-json.test.js'
...
- [铁律10] 适用：B-01 至 B-05 是本 Sprint smoke，并须入 test-registry.yaml。
...
- [铁律21] 适用：push 前执行入册检查、product-map:check 与定向测试。
- [铁律22] 适用：B-01 至 B-05 是本 Sprint smoke。
- [铁律23] N/A：本 Sprint 仅改本地只读 CLI 输出与单测，不触及该铁律对应的服务、DB、调度、真机、部署或外部系统。
- [铁律24] 适用：B-01 至 B-05 是本 Sprint smoke。
...
- [铁律34] 适用：B-01 至 B-05 是本 Sprint smoke。

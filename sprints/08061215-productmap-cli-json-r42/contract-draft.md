...
**可观测行为**: 隔离副本缺少或含损坏 `product-map.json` 时，stdout 仍是失败 JSON，errors 给出具体原因且 stderr 无裸堆栈。

**验证命令**: `node --test --test-name-pattern='缺少 product-map.json|不可解析 product-map.json' sprints/08061215-productmap-cli-json-r42/tests/product-map-cli-json.test.js`

**硬阈值**: 两种故障均 `ok=false`、errors 非空且逐项 string、退出码非 0、stdout 可 JSON.parse；上述命令非零即 FAIL。
...
### Step 4: JSON 参数与既有参数并存

**来源**: `[FROM_PRD]` — Thin PRD「边界情况」第二条。

**可观测行为**: `check --json --verbose` 仍输出相同 JSON 检查结论，不发生参数位置干扰。

**验证命令**: `node --test --test-name-pattern='其他既有参数并存' sprints/08061215-productmap-cli-json-r42/tests/product-map-cli-json.test.js`

**硬阈值**: 退出码 0 且 JSON 严格等于 `{ok:true,errors:[]}`；上述命令非零即 FAIL。

### Step 5: 调用方用退出码完成门禁

**来源**: `[FROM_PRD]` — Thin PRD「Golden Path」具体第 3 条。

**可观测行为**: 同一真实 CLI 在有效输入返回 0，在缺失/损坏输入返回非 0，JSON 内容与退出码语义一致。

**验证命令**: `node --test sprints/08061215-productmap-cli-json-r42/tests/product-map-cli-json.test.js`

**硬阈值**: 5 个子测试全部通过且 node:test 进程退出 0；否则 FAIL。
...
grep -q '# pass 5' /tmp/product-map-cli-json-e2e.log
...
| 参数并存 | `tests/product-map-cli-json.test.js` | `与其他既有参数并存时仍返回同一 JSON 结论` | 当前实现未识别 `--json` |
...
- 边界值: `--json` 位于 `check` 后不同位置并与未知透传参数并存；errors 多项时均为 string。

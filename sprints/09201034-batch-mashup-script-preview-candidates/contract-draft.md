...
> **Round 2 修订（针对 R1 两个 blocker，先减后加、净变化 ≈0）**：
> - **R1-1**（verification_oracle_completeness）：为 PRD 验收#3「虚拟滚动 DOM 节点数与可视区一致、非 200+ 全量渲染」补 **playwright 硬断言 oracle**（见 ## E2E 验收 变体C 的「虚拟滚动 DOM 节点数硬断言」小节 + DoD B-10），不再仅靠截图目检。
> - **R1-2**（ci_workflow_alignment）：明确要求内嵌 PowerShell 以文件名 **`${SPRINT_DIR}/e2e-verify.ps1`** 落盘（e2e-windows.yml 唯一入口），playwright spec 以 **`apps/dashboard/e2e/mashup.spec.ts`** 落盘，并在 ARTIFACT 登记 + 机检存在/内容（见 ## ARTIFACT 补充 与 DoD ARTIFACT）。
> 除以上两处外，合同内容与 Round 1 保持一致（无 scope 蔓延）。
...
> `gh workflow run e2e-windows.yml --repo perfectuser21/zenithjoy-workspace -f sprint_dir=${SPRINT_DIR} -f task_id=${TASK_ID}`。spec 禁 `page.route()`，所有请求打真实 apps/api（port 3000）。验收点：MashupPage 粘贴文案见动态分段、PickStep `<video>` 可播放、CandidatesStep 虚拟滚动（DOM 节点数≈可视区，非 200 全量）、选中候选后见渲染队列/失败态。截图存 `${SPRINT_DIR}/screenshots/staging-<step>.png`。
>
> **【R1-2 落盘死规则】CI 链路闭合要求（CONTRACT IS LAW，Generator 必须照此落盘两文件）**：
> `.github/workflows/e2e-windows.yml` 的**唯一执行入口硬编码为 `${sprint_dir}/e2e-verify.ps1`**（该文件不存在即 `Write-Error` + `exit 1`）。因此：
> 1. 下面这段 PowerShell **必须以文件名落盘为 `sprints/09201034-batch-mashup-script-preview-candidates/e2e-verify.ps1`**（不得改名、不得只内嵌于合同）；
> 2. playwright spec **必须以 `apps/dashboard/e2e/mashup.spec.ts` 落盘**（与 ps1 里 `playwright test e2e/mashup.spec.ts` 逐字一致）；
> 3. 两文件均在 ## ARTIFACT 补充 与 contract-dod.md ARTIFACT 段登记并机检存在/内容。缺任一 → e2e-windows.yml `exit 1` → PRD 验收#5「CI 全绿」不可达。

```powershell
# 落盘路径：sprints/09201034-batch-mashup-script-preview-candidates/e2e-verify.ps1
...
$p = Start-Process -FilePath "cmd.exe" -ArgumentList "/c npx.cmd playwright test e2e/mashup.spec.ts --project=chromium --reporter=list" -WorkingDirectory "$repoRoot\apps\dashboard" -Wait -PassThru -NoNewWindow
if ($p.ExitCode -ne 0) { throw "FAIL: Playwright E2E exit=$($p.ExitCode)" }
Write-Host "OK: windows_cloud Dashboard E2E 通过（真实后端）"
```

### 虚拟滚动 DOM 节点数硬断言（R1-1 — PRD 验收#3 的可执行 oracle，写入 `apps/dashboard/e2e/mashup.spec.ts`）

**背景**：PRD 验收#3 要求「前端虚拟滚动渲染 DOM 节点数与可视区域一致，非 200+ 全量渲染」——这是**可测量**行为。仅靠截图目检无法测量 DOM 节点数，一个把全部候选全量渲染的非虚拟化实现同样能过图检。故 `mashup.spec.ts` 必须包含以下**硬断言**（环境可控，不依赖真 ffmpeg 抽帧接缝）：

- 前置：为使断言确定可复现，spec 通过真实后端（apps/api，禁 `page.route()`）准备一个 `generatedCount` 很大的 run（≥ 60 条候选，例如经真 API 播种/生成后 `GET /runs/:id/candidates` 返回的 `data.generatedCount`）；候选卡片带稳定标识 `data-testid="candidate-card"`。
- Generator 必须在 `CandidatesStep` 的每张候选卡渲染 `data-testid="candidate-card"`，虚拟滚动仅挂载可视区附近节点。
- 硬断言（伪代码，Generator 落成真实 playwright 断言）：

```javascript
// apps/dashboard/e2e/mashup.spec.ts — 虚拟滚动 DOM 节点数硬断言（禁 page.route，打真实后端）
await page.goto(`${BASE}/mashup`);
// ...走到 candidates 步骤，run 的 generatedCount 大（≥60）
const shown = await page.getByText(/共生成\s*\d+\s*条候选/).innerText();
const generatedCount = parseInt(shown.match(/(\d+)/)[1], 10);
expect(generatedCount).toBeGreaterThanOrEqual(60);           // 大候选量前提成立
const domCards = await page.locator('[data-testid="candidate-card"]').count();
expect(domCards).toBeLessThan(generatedCount);               // 关键：非全量渲染
expect(domCards).toBeLessThanOrEqual(40);                    // ≈可视区容量上限（远小于总数）
```

- **PASS**：`generatedCount≥60` 且 `domCards < generatedCount` 且 `domCards ≤ 40`（虚拟化落地）。
- **FAIL（非虚拟化实现被抓）**：`domCards == generatedCount`（全量渲染 200 个节点）→ 断言失败 → playwright exit≠0 → e2e-windows.yml `exit 1`。

---

## ARTIFACT 补充（R1-2 — CI 链路两文件，Generator 必须落盘）

| 文件 | 作用 | 机检（见 contract-dod.md ARTIFACT 段）|
|---|---|---|
| `sprints/09201034-batch-mashup-script-preview-candidates/e2e-verify.ps1` | e2e-windows.yml 唯一入口（缺则 exit 1）；启 apps/api + vite preview + playwright | 存在且含 `vite preview`、`playwright test e2e/mashup.spec.ts`、`apps\api` |
| `apps/dashboard/e2e/mashup.spec.ts` | UI 层 E2E：文案分段 / `<video>` 预览 / **虚拟滚动 DOM 节点数硬断言** / 渲染队列态 | 存在且含 `candidate-card`、`toBeLessThan`（DOM 节点数硬断言存在）、无 `page.route(` |

链路闭合：`gh workflow run e2e-windows.yml` → checkout → `${sprint_dir}/e2e-verify.ps1` → `playwright test e2e/mashup.spec.ts` → DOM 节点数硬断言 → PRD 验收#5「CI 全绿」。

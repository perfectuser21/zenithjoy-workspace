...
> 允许 mock 的外层无关边界：embedding 模型（付费/网络）、TOAPIS/Gemini（Step1 由 deps.callAi 注入替身，真调一次由 real-toapis-segment.mjs 覆盖）、ffmpeg 二进制（真出片在 windows_cloud E2E）、COS 存储（InMemoryMaterialStorage）。
...
|---|---|---|
| Step1 happy-path TOAPIS/Gemini 真调（合同单测里由 deps.callAi 替身） | 控成本 + 单测确定性；真 key 可能未注入 CI secret | real-toapis-segment.mjs 用真 key 真调一次（规则B）；若 evaluator 环境无 `TOAPIS_API_KEY`，该条**不静默假绿**、直接 FAIL 并登记为本轮真实链路缺口，须由主理人在 staging（生产已在用 ~/.credentials/toapis.env）复核 Step1 真解析 |
...
node "${SPRINT_TESTS}/real-toapis-segment.mjs"
...
|---|---|---|---|
| Step1 文案动态分段（真库写路径） | `sprints/09201034-batch-mashup-script-preview-candidates/tests/mashup-script-segment.test.ts` | 落成新 mashup_templates 行、命中 ai_tags 枚举、fallbackMapped=true、静默降级固定模板、只真调一次 AI、name 截断到列约束内、空文案 → 抛 INVALID_BODY | mashup-script-segment.ts / ai-tags.ts 模块不存在 → import/collect error → N failures |
...
| Step4 渲染并发=1 队列（并发接缝） | `sprints/09201034-batch-mashup-script-preview-candidates/tests/mashup-render-queue.test.ts` | 并发上限常量=1、第二个并发请求进入 queued 且带 queuePosition=1、渲染函数失败 → render_failed 态 | mashup-render-queue.ts 模块不存在 → import error → N failures |
| Step2 素材在线预览重签（读路径） | `sprints/09201034-batch-mashup-script-preview-candidates/tests/mashup-materials-preview.test.ts` | 本租户素材 200 返回 preview_url、非本租户 404、无凭据 401 | GET /:id/preview-url 未注册（404 Not Found）→ 断言 failures |
| Step1 第三方真调一次（规则B） | `sprints/09201034-batch-mashup-script-preview-candidates/tests/real-toapis-segment.mjs` | 真 TOAPIS 返回结构化分段（含 roleLabel） | 端到端脚本，实现前配套 prompt/解析未定型 → 非 0 退出 |

> BEHAVIOR 覆盖列每个覆盖名均为对应 it() 测试名的字面子串（供下游字符串映射）。
> Test File 列为完整真实路径（无省略号），封印闸 assertTestContractResolvable 可解析。

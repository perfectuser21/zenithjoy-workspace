...
```
**硬阈值**: 并发 2 个请求，至少 1 个 `renderStatus:queued` 且 `queuePosition≥1`（并发=1 生效，E2E Step4b + oracle `s4-concurrency.cjs` 验）；真 ffmpeg 合成成片 1920x1080（E2E Step4a `concatAndScale`+`ffprobe` **实测** green oracle，口径同 render-smoke 合成层，无需 key）；渲染失败→ `render_failed` 且可重试（E2E Step4b + oracle `s4-failed.cjs` 验）。**未覆盖（登记规则C）**：内容安全通过→真上传成片 `exportUrl` 完整交付路径依赖真 TOAPIS/Gemini 调用，见「未覆盖真实链路清单」补位计划
...
| 内容安全通过 → 真上传成片 `exportUrl` 完整交付（1080P 成片端到端落地） | 该路径需内容安全 fail-closed **通过**（真 TOAPIS/Gemini 调用），成本高 + 外部服务抖动会让基线闸假红——与 `mashup-render-smoke.sh` 自身声明「不覆盖『安全审核通过→真上传成片』」逐字同口径。**真 ffmpeg 合成 1920x1080 已由 E2E Step4a green oracle 覆盖**；此处未覆盖的仅是「安全通过后的上传交付 + exportUrl 非空」这一段 | E2E 在 `TOAPIS_API_KEY` 存在时执行有 key 分支：轮询 `render-jobs` 至 `rendered` + 断言 `exportUrl` 非空 + `safetyCheckStatus=passed`；由 CI has-key 专门 job / 生产覆盖，同 render-smoke 有 key job 口径 |
...
# 4a. 真 ffmpeg 合成层 green oracle：多段不同分辨率素材统一到 1920x1080（backing Step4 硬阈值的
#     「真实 ffmpeg 渲染成片」；口径同 mashup-render-smoke.sh Step1，用现有未改的 concatAndScale，
#      无需 TOAPIS key —— PRD 验收标准4「真实 ffmpeg 渲染」的 green 成功路径就落在这里）。
RENDER_OK=$(node -e "const { concatAndScale } = require('./apps/api/dist/services/mashup-render-ffmpeg.js'); console.log(concatAndScale(['$WORKDIR/a.mp4', '$WORKDIR/b.mp4'], '$WORKDIR/out.mp4') ? 'yes' : 'no');")
[ "$RENDER_OK" = "yes" ] || fail "concatAndScale 返回 false（真 ffmpeg 合成失败）"
[ -s "$WORKDIR/out.mp4" ] || fail "ffmpeg 渲染成片未生成"
DIMS=$(ffprobe -v error -select_streams v -show_entries stream=width,height -of csv=p=0 "$WORKDIR/out.mp4")
[ "$DIMS" = "1920,1080" ] || fail "渲染成片分辨率非 1920x1080，实际=$DIMS（违反 PRD 1080P 硬阈值 / 累积FR 不回退）"
ok "Step4a 真 ffmpeg 渲染成片 1920x1080 验证通过（ffprobe 实测 green oracle）"
# 4b. 并发=1 排队 + 渲染失败态可重试 + fail-closed（未配 key → 走 fail-closed 分支）

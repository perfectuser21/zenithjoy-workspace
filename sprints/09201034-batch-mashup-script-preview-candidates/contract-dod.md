...
- [ ] [ARTIFACT] (R1-2) e2e-windows.yml 唯一入口 e2e-verify.ps1 已落盘该 sprint 路径，含 vite preview + playwright mashup.spec.ts 调用
  Test: node -e "const c=require('fs').readFileSync('sprints/09201034-batch-mashup-script-preview-candidates/e2e-verify.ps1','utf8');if(!c.includes('vite preview')||!c.includes('playwright test e2e/mashup.spec.ts')||!c.includes('apps\\\\api'))process.exit(1)"
  期望: exit 0

- [ ] [ARTIFACT] (R1-1/R1-2) UI 层 E2E spec apps/dashboard/e2e/mashup.spec.ts 存在，含虚拟滚动 DOM 节点数硬断言且禁 page.route()
  Test: node -e "const c=require('fs').readFileSync('apps/dashboard/e2e/mashup.spec.ts','utf8');if(!c.includes('candidate-card')||!c.includes('toBeLessThan')||c.includes('page.route('))process.exit(1)"
  期望: exit 0
...
## BEHAVIOR:E2E 条目（user_facing 专属，Mode B final-e2e 在 staging / windows_cloud 跑）

- [ ] [BEHAVIOR:E2E] [L3] B-10: 候选虚拟滚动 DOM 节点数硬断言（PRD 验收#3 — 非全量渲染）[接缝×2]
  动作: playwright 打真实后端走到 candidates 步骤，run 的 generatedCount 大（≥60），读取「共生成 N 条候选」并 count 候选卡 DOM 节点
  预期观察: 页面显示的 generatedCount≥60，但实际渲染的 `[data-testid="candidate-card"]` DOM 节点数 < generatedCount 且 ≤40（≈可视区容量），证明虚拟化生效、非 200+ 全量渲染
  等待预算: 60s（candidates 加载 + 虚拟列表首屏挂载）
  留证: apps/dashboard/e2e/mashup.spec.ts playwright 报告 + ${SPRINT_DIR}/screenshots/staging-03-candidates.png
  Test: manual:bash -c 'node -e "const c=require(\"fs\").readFileSync(\"apps/dashboard/e2e/mashup.spec.ts\",\"utf8\");if(!/toBeLessThan\(\s*generatedCount/.test(c)||!/candidate-card/.test(c)){console.error(\"FAIL: mashup.spec.ts 缺 DOM 节点数<generatedCount 硬断言\");process.exit(1)}console.log(\"OK: 虚拟滚动 DOM 硬断言存在\")"'

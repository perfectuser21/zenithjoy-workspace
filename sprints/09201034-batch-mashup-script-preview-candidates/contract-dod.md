...
> **oracle 说明**：DB-heavy 行为的可执行 oracle 以冻结脚本 `tests/oracles/*.cjs` 承载（避免 Test 单行嵌套引号出错）；evaluator 直接 `node` 跑，脚本内含真 pool 真断言，实现缺失时 RED。轻量行为直接单行 node/psql。深层全链路见 contract-draft.md `## E2E 验收`。
...
- [ ] [BEHAVIOR] [L2] B-01: 粘贴文案→生成动态分段模板，AI 不可用时静默降级固定模板且非阻断（INV-4 + INV-2）
  ...
  Test: manual:bash -c 'DATABASE_URL="$DB_URL" node sprints/09201034-batch-mashup-script-preview-candidates/tests/oracles/s1-fallback.cjs'
...
- [ ] [BEHAVIOR] [L2] B-02: 空 script 被拒绝，不触碰 DB（error path）
  ...
  Test: manual:bash -c 'node sprints/09201034-batch-mashup-script-preview-candidates/tests/oracles/s1-empty.cjs'
...
- [ ] [BEHAVIOR] [L2] B-03: 素材在线预览 URL 可签出（前端 `<video>` 数据源）
  ...
  等待预算: 0s
  留证: 命令 stdout（URL 前缀）
  Test: manual:bash -c 'node sprints/09201034-batch-mashup-script-preview-candidates/tests/oracles/s2-preview.cjs'
...
- [ ] [BEHAVIOR] [L2] B-04: 候选生成诚实计数——generatedCount == candidates.length == DB落库数 且 ≤ targetCount，不凑数（INV-1）
  ...
  Test: manual:bash -c 'DATABASE_URL="$DB_URL" node sprints/09201034-batch-mashup-script-preview-candidates/tests/oracles/s3-honest.cjs'
...
- [ ] [BEHAVIOR] [L2] B-06: 选中候选→真实渲染并发上限=1，第 2 个并发请求进排队态 queued 第 N 位 [接缝×2]
  ...
  Test: manual:bash -c 'DATABASE_URL="$DB_URL" node sprints/09201034-batch-mashup-script-preview-candidates/tests/oracles/s4-concurrency.cjs'
...
- [ ] [BEHAVIOR] [L2] B-07: 渲染失败→render_failed 态可重试，且安全未过 fail-closed 无 export_url（PRD Step4 + INV-3）[接缝×2]
  ...
  Test: manual:bash -c 'DATABASE_URL="$DB_URL" node sprints/09201034-batch-mashup-script-preview-candidates/tests/oracles/s4-failed.cjs'

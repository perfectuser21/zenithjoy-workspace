...
describe('Kernel acquisition effective-config guard [BEHAVIOR]', () => {
  ...
  it('合法部分 PATCH 只改变请求字段且保持双租户隔离', async () => {
    const beforeA = withoutUpdatedAt(await readConfig(tenantA));
    const beforeB = await readConfig(tenantB);
    ...
    const afterA = withoutUpdatedAt(await readConfig(tenantA));
  ...
  it('合法完整 PUT 含全部配置字段且 min=max 时整行持久化可读回', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.success).toBe(true);
    expect(response.body.data).toMatchObject({ tenant_id: tenantA, ...completeConfig });
    expect(withoutUpdatedAt(await readConfig(tenantA))).toMatchObject({ tenant_id: tenantA, ...completeConfig });
    expect(await readConfig(tenantB)).toEqual(beforeB);
  });
  ...
  it('已有租户并发部分更新按实际可见配置串行校验且最终合法', async () => {
    ...
    const responses = await Promise.all([
      request(app).patch('/api/acquisition/config').set('X-Tenant-Id', tenantA)
  ...
  it('新租户首次并发 upsert 串行校验且不会创建无效有效态', async () => {
    expect(await readConfig(tenantNew)).toBeUndefined();
    const beforeB = await readConfig(tenantB);

    const responses = await Promise.all([
      request(app).patch('/api/acquisition/config').set('X-Tenant-Id', tenantNew)
    ...
    expect(responses.map((response) => response.status).sort()).toEqual([200, 400]);
    expectInvalidConfig(responses.find((response) => response.status === 400)!);
    const finalNew = await readConfig(tenantNew);

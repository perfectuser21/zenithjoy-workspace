...
- [x] [ARTIFACT] `knowledgeAuthGuard` 中间件存在且源码零身份头名（A27 扫描目标本体）
...
- [x] [ARTIFACT] 员工目录模块存在且实现四项自检标识（A30-1a / A30-1b / A30-2 / A30-3）
...
- [x] [ARTIFACT] 投影表 migration 存在于 `apps/api/db/migrations/`，含 `org_id` 且 NOT NULL，DDL 幂等
...
- [x] [ARTIFACT] 前端知识面用独立 `knowledgeFetch`（只带 cookie，零身份头），且不复用 `adminFetch`
...
- [x] [ARTIFACT] smoke 脚本落盘且**已登记进 baseline**（未登记 = 只 warning 不闸，仓库实测 C11）
...
- [x] [ARTIFACT] windows_cloud 车道入口 `e2e-verify.ps1` 存在，且 UI spec 禁 `page.route()`
...
- [x] [ARTIFACT] 假飞书上游带生产门禁（`NODE_ENV=production` 一律 404），沿用本仓既有 `_smoke-fake-*` 模式
...
- [x] [BEHAVIOR] 四项成立时 API 起得来，**且启动日志证明自检真跑过**（只验"服务起来了"是假绿——没实现自检时服务照样起）
...
- [x] [BEHAVIOR] A30 四条变异各自报红且日志指明违规项（proven-to-fire，缺一条不算）
...
- [x] [BEHAVIOR] 白名单员工登录 → `Set-Cookie` 三属性齐全（httpOnly / Secure / SameSite=Lax）
...
- [x] [BEHAVIOR] 成员行挂在**声明组织**下、未落 Personal-% 租户、登录未新建租户
...
- [x] [BEHAVIOR] 员工目录无归属声明的账号 → 403 NO_ORG_ASSIGNMENT，无 Set-Cookie，零成员行
...
- [x] [BEHAVIOR] 无会话调知识端点 → 401 SESSION_REQUIRED，文案「登录已失效，请重新登录」
...
- [x] [BEHAVIOR] 伪造 `X-User-Email` + `X-Feishu-User-Id` 头调录入 → 判定不变（仍 401）且账本零新增（本 GP 命门）
...
- [x] [BEHAVIOR] 有会话但无成员行 → 403 NO_TENANT，文案「没有权限」，与 401 文案不同
...
- [x] [BEHAVIOR] 录入成功 → 201，`data.org_id` 等于声明组织，账本本轮真有该行且带 org_id 与 author_member_id
...
- [x] [BEHAVIOR] 录入响应 schema 完整且禁用字段一个不出现（防语义漂移）
...
- [x] [BEHAVIOR] 缺组织上下文 → 403 NO_ORG_CONTEXT 且账本零新增（fail-closed，不写无归属行）
...
- [x] [BEHAVIOR] 非法 `evidence_url`（非 http/https）→ 400，且账本零新增（error path + 输入对抗面）
...
- [x] [BEHAVIOR] 请求体里塞 `org_id` 想覆盖归属 → 被忽略，落库归属仍为会话声明组织（越权指令拒绝）
...
- [x] [BEHAVIOR] 提交后 30 秒内在「最近沉淀」读到**那一条**（按 entry_id 精确命中，非"列表非空"）且证据链接逐字回读
...
- [x] [BEHAVIOR] 「最近沉淀」列表零跨组织条目，且响应 schema keys 完整（跨企业硬隔离）
...
- [x] [BEHAVIOR] A27 守卫正向通过；往 `knowledgeAuthGuard` 加回一行读头后必须报红，且源码已还原
...
- [x] [BEHAVIOR] `adminFetch` 仍拼两个身份头、`staffGuard` 相对 base_sha 零 diff、端点计数字面等于 16
...
- [x] [BEHAVIOR] INV-1 [身份来自会话] 同一请求，只换 cookie 不换头 → 判定跟着会话走；只换头不换 cookie → 判定纹丝不动
...
- [x] [BEHAVIOR] INV-2 [跨企业硬隔离] 用企业B 会话读「最近沉淀」→ 看不到企业A 那条，且其列表零企业A 条目
...
- [x] [BEHAVIOR] INV-5 [SSOT 单向] 投影表**只读读端真的存在**（GET 200）而写端点不存在（POST 404/405），源码级零写入语句
...
- [x] [BEHAVIOR] INV-12 [不静默降级] 账本不可达时读端回 503 LEDGER_UNREACHABLE，绝不返回空列表冒充「库里还没有」
...
- [x] [BEHAVIOR] INV-G [守卫非空] A27 与 A30 合计五条变异全部 proven-to-fire（本 sprint 合入前置，GP 合同 stages 硬要求）
...
- [x] [BEHAVIOR:E2E] 员工在真实浏览器走完 Golden Path：登录 → 录入 → 「最近沉淀」页看到本人这条带证据链接（**真派发真跑 windows_cloud 车道**，不是静态 grep）

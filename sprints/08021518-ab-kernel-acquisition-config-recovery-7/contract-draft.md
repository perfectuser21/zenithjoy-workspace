...
- `error.message` 为非空字符串，但不得回显完整租户配置。
...
**可观测行为**: 只改 min、只改 max、完整更新及 min=max 均成功，GET/DB 返回合并后的新值，B 租户保持不变。
...
**硬阈值**: HTTP 200，响应与真 DB 中两个值均为 7；命令用 `jq -e` 和 SQL 定点读共同执法。
...
for ATTEMPT in 1 2; do
  ...
  [ "$CODE" = 400 ]
  jq -e '.success==false and .error.code=="INVALID_CONFIG" and (.error.message|type=="string") and (keys==["error","success","timestamp"]) and (has("error_code")|not)' "$BODY"
...
curl -sf -X PUT "$API_URL/api/acquisition/config" -H "X-Feishu-User-Id: $TENANT_A_USER" -H 'Content-Type: application/json' -d '{"keywords_per_round_min":7,"keywords_per_round_max":7}' | jq -e '.success==true and .data.keywords_per_round_min==7 and .data.keywords_per_round_max==7'
curl -sf "$API_URL/api/acquisition/config" -H "X-Feishu-User-Id: $TENANT_A_USER" | jq -e '.data.keywords_per_round_min==7 and .data.keywords_per_round_max==7'
...
|---|---|---|---|
| 合并后配置校验与租户隔离 | `tests/acquisition-config-effective-validation.integration.test.ts` | `只更新最小值且合并后倒置时拒绝并零持久化`；`有效部分、完整和相等边界更新继续持久化且不串租户` | 当前 `upsertConfig` 只合并后写入而不校验，首条断言失败 |

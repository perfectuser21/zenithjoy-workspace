...
| 风险 | Mitigation |
|---|---|
| 读取—校验—写入间并发更新造成 TOCTOU，后到写覆盖先到写 | 实现必须在同一数据库事务内锁定当前租户配置行（或等价原子方案）；integration test 真实触达读写边，禁止 mock。 |
| 服务层 INVALID_CONFIG 未被路由捕获而变成 HTTP 500 | 真 HTTP integration test 断言状态恰为 400 和嵌套 `error.code`；PUT 与既有 PATCH 调用点均稳定映射该领域错误。 |

## 禁 mock 边清单

- HTTP 路由 ↔ `upsertConfig`（必须以真 HTTP 证明领域错误映射为 400）。
- `upsertConfig` ↔ 真 Postgres `zenithjoy.acquisition_config`（必须验证读取、合并、拒绝零写入）。
...
**验证命令**: `F=$(mktemp); C=$(curl -sS -o "$F" -w '%{http_code}' -X PUT "$API_URL/api/acquisition/config" -H "X-Tenant-Id: $TENANT_A_ID" -H 'Content-Type: application/json' -d '{"keywords_per_round_min":9}'); [ "$C" = 400 ] && jq -e '.success==false and .error.code=="INVALID_CONFIG"' "$F"`
**硬阈值**: HTTP 恰为 400、错误码字面匹配，由状态码与 jq 共同执法。
...
### Step 4: 有效部分、完整及相等边界继续成功
**来源**: `[FROM_PRD]` — PRD Golden Path 第 3 步与边界情况。
**可观测行为**: min-only、max-only、完整更新与 min=max 返回 200 并可从 DB 读到，B 不变。
**验证命令**: `curl -sf -X PUT "$API_URL/api/acquisition/config" -H "X-Tenant-Id: $TENANT_A_ID" -H 'Content-Type: application/json' -d '{"keywords_per_round_min":7,"keywords_per_round_max":7}' | jq -e '.success==true and .data.keywords_per_round_min==7 and .data.keywords_per_round_max==7'`
**硬阈值**: HTTP 200 且两值均为 7，由上述命令执法。

### Step 5: 冻结 Red 与盲测边界
**来源**: `[AI_ADDED]` — 将 PRD 的测试/盲测约束变为交付边界，防止假绿。
**可观测行为**: 不修改共享 Red fixture，不检查 One-session 候选材料，不在 verdict 前合并。
**验证命令**: `git diff --exit-code 0dc4e3c07ff19a0ac95440723986bf3cb78580b2 -- shared-red-fixture 2>/dev/null || [ ! -e shared-red-fixture ]`
**硬阈值**: 共享 Red fixture 零 diff。

## 接缝清单

- [接缝×2] 真 HTTP ↔ 服务 ↔ 真 Postgres：无效 min-only/max-only 各重复两次，均须 400/INVALID_CONFIG 且零写入。
- 读取—校验—写入并发边：真 Postgres 事务/锁行为验证前为 `logic-done-pending`，不得标 done。
...
for PAYLOAD in '{"keywords_per_round_min":9}' '{"keywords_per_round_max":2}'; do
  for ATTEMPT in 1 2; do
    ...
    [ "$C" = 400 ]
    jq -e '.success==false and .error.code=="INVALID_CONFIG" and (.error.message|type=="string" and length>0) and (keys==["error","success","timestamp"]) and (has("error_code")|not) and (has("code")|not)' "$F"
...
curl -sf -X PUT "$API_URL/api/acquisition/config" -H "X-Tenant-Id: $TENANT_A_ID" -H 'Content-Type: application/json' -d '{"keywords_per_round_min":7}' | jq -e '.success==true and .data.keywords_per_round_min==7 and .data.keywords_per_round_max==8'
curl -sf -X PUT "$API_URL/api/acquisition/config" -H "X-Tenant-Id: $TENANT_A_ID" -H 'Content-Type: application/json' -d '{"keywords_per_round_max":9}' | jq -e '.success==true and .data.keywords_per_round_min==7 and .data.keywords_per_round_max==9'
curl -sf -X PUT "$API_URL/api/acquisition/config" -H "X-Tenant-Id: $TENANT_A_ID" -H 'Content-Type: application/json' -d '{"keywords_per_round_min":7,"keywords_per_round_max":7}' | jq -e '.success==true and .data.keywords_per_round_min==7 and .data.keywords_per_round_max==7'
...
|---|---|---|---|
| 合并校验、HTTP 映射、零写入 | `tests/acquisition-config-effective-validation.integration.test.ts` | `真 HTTP 对合并后无效的 min-only patch 返回 400 INVALID_CONFIG 且零持久化`；`有效部分、完整和相等边界更新继续成功且不串租户` | 冻结实现第一条收到 200 而非 400 |

...
**可观测行为**: 真实共享 Red 文件保持冻结 SHA 的 blob；本轮变更只允许落在 PRD 指定的 `apps/api/` 实现/测试范围与本 sprint 合同目录，不检查或复制 One-session 候选材料；盲测裁决前不存在 merge commit。
**验证命令**: `BASE=18333a8293ea3a9c8fac1a3111142fa6491fbb59; RED=0dc4e3c07ff19a0ac95440723986bf3cb78580b2; RED_PATH=apps/api/tests/routes/acquisition-dispatch.test.ts; test "$(git rev-parse "$RED:$RED_PATH")" = "$(git hash-object "$RED_PATH")" && test -z "$(git diff --name-only "$BASE" HEAD -- . ':(exclude)apps/api/**' ':(exclude)sprints/08021518-ab-kernel-acquisition-config-recovery-7/**')" && test -z "$(git rev-list --min-parents=2 "$BASE"..HEAD)"`
**硬阈值**: 共享 Red 文件 blob 与冻结 SHA 完全相同；base 之后零 `apps/api/` 与本 sprint 目录以外变更；base 之后零 merge commit，三项均由上述命令执法。

...
- [ ] [BEHAVIOR] [L2] B-02: 错误 schema 完整且不误映射为 500
  ...
  预期观察: 状态恰为 400；顶层仅 error/success/timestamp；timestamp 可按 ISO-8601 解析；error.code 为 INVALID_CONFIG 且 error.message 为非空字符串
  等待预算: 30s
...
- [ ] [BEHAVIOR] [L2] B-05: 冻结 Red 与盲测 A/B 仓库边界保持完整
  ...
  预期观察: 共享 Red 文件与冻结 SHA 完全一致；变更仅位于 PRD 指定的 apps/api 与本 sprint 合同目录；盲测裁决前没有 merge commit
  等待预算: 0s

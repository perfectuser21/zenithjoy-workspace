# Sprint PRD — acquisition 合并配置校验恢复

## OKR 对齐

- **对应 KR**：未解析到活跃 KR
- **当前进度**：待定（Brain context 未返回活跃 OKR）
- **本次推进预期**：恢复 Line02 keyword_acquisition Step7 的配置更新校验合同

## 背景

从冻结 Red SHA `0dc4e3c07ff19a0ac95440723986bf3cb78580b2` 出发，修复 acquisition 部分更新绕过合并后配置校验的问题。此恢复样本不视为 frozen-equivalent，既往 Kernel 失败与基础设施修复耗时继续计入 Kernel。

## Golden Path（核心场景）

租户从 acquisition 配置更新入口提交部分或完整配置 → 系统与该租户当前配置合并并校验有效配置 → 无效更新被拒绝且不落库，合法更新正常保存。

具体：
1. 租户提交只包含 `keywords_per_round_min` 或 `keywords_per_round_max` 的部分更新，或同时包含两者的完整更新。
2. 系统以补丁和当前租户配置合并后的有效配置为校验对象；当 `keywords_per_round_min > keywords_per_round_max` 时拒绝请求。
3. 无效有效配置返回 HTTP 400，且响应 `error.code` 为 `INVALID_CONFIG`；原配置保持不变。
4. 合并后合法的部分更新与合法完整更新继续成功并持久化，不发生回归。

## 边界情况

- 单独提高最小值后超过现有最大值，必须拒绝且不持久化。
- 单独降低最大值后低于现有最小值，必须拒绝且不持久化。
- 补丁字段本身看似合法、但与租户当前值合并后非法，仍按无效有效配置处理。
- 边界值相等（最小值等于最大值）仍属于合法配置。
- 校验和读写必须限定到当前租户，测试至少覆盖两个租户且互不串写。

## 范围限定

**在范围内**：acquisition 配置部分/完整更新的合并后校验、HTTP 400 + `INVALID_CONFIG` 错误合同、无效更新零持久化、合法更新回归保护、先写失败 smoke test。
**不在范围内**：修改共享 Red fixture；查看或复制 One-session 候选 worktree、patch、日志、PR 或反馈；盲测 A/B verdict 前合并；扩展其他配置字段或 UI。

## 假设

- [ASSUMPTION: `gp_anchor=line02/keyword_acquisition#step7` 是本 sprint 的 Golden Path 唯一步骤锚点。]
- [ASSUMPTION: payload 明确指定 `target_environment=local_api`，由本地 API 测试覆盖入口到持久化出口。]
- [ASSUMPTION: task description 作为 PrepPRD 主源，已足以锚定缺失的 `payload.thin_prd`。]

## 预期受影响文件

- `apps/api/` 中 acquisition 配置更新入口：保证按租户当前配置合并后再判定有效性。
- `apps/api/` 对应 smoke/集成测试：先冻结无效部分更新的 HTTP 与零持久化行为，再保护合法更新。
...
- 频控: 待定（PrepPRD 未指定）
- 版本要求: 以冻结 Red SHA `0dc4e3c07ff19a0ac95440723986bf3cb78580b2` 为起点
- 可观测: 无效配置必须以 HTTP 400 和 `error.code=INVALID_CONFIG` 明确可观察，且存储状态不变
...
- [多设备类型(os_t] 1) contract-dod模板加规则：新字段与既有字段语义重叠时必须本sprint内消解或建正式decision+挂任务队列，禁止只在文档里写'留给后续技术债sprint'了事，harness-contract-reviewer遇到此类表述直接判needs_revision；2) harness-planner 4问加第5问：涉及几种设备/操作系统类型？每种是否都有对应UI区分？3) golden-path-reviewer 6维rubric加'多端完整性'维度：功能涉及多个os_type/device_platform时验收需确认展示层是否区分，不区分则FAIL；4) 已排一次全仓一次性扫描找同类'字段有但下游UI未接线'模式。（来源: area）
...
<!-- 来源: journey_id 缺失，无法加载 line golden paths -->
- （本 line 暂无历史）
...
# 期望验收点：先以失败测试证明两种非法部分更新均返回 HTTP 400/INVALID_CONFIG 且租户原配置不变；
# 再证明最小值等于最大值、合法部分更新及合法完整更新可成功持久化，并验证双租户隔离。
```

## journey_type: autonomous
## journey_type_reason: 任务是纯后端 acquisition 配置更新与持久化行为修复，无 UI 或远端 agent 协议。
## target_environment: local_api
## target_environment_reason: payload 显式指定 local_api，由本地 API 与隔离测试验证 HTTP 和持久化出口。
## journey_id: none

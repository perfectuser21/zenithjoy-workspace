...
**范围**: `.github/workflows/agent-e2e-video.yml` 的 `agent_version` input `default:` 改为 `"1.1.31"`（与 `services/agent/package.json` version 对齐）
**大小**: S（~3 行净增，1 文件）
**依赖**: Workstream 2 完成后
...
- [ ] [ARTIFACT] `agent-e2e-video.yml` 的 `default:` 字段值更新为 "1.1.31"
  Test: node -e "const c=require('fs').readFileSync('.github/workflows/agent-e2e-video.yml','utf8');const m=c.match(/default:\s*\"([^\"]+)\"/);if(!m){process.exit(1)}if(m[1]!=='1.1.31'){console.error('FAIL: default='+m[1]+' 非 1.1.31');process.exit(1)}console.log('ARTIFACT OK')"

## BEHAVIOR 条目

### BEHAVIOR 1: GHA default 等于 agent package.json version — schema 字段值
...
### BEHAVIOR 2: GHA default 具体值为 "1.1.31" — schema 完整性
...
### BEHAVIOR 3: GHA default 不含旧版 1.1.30 — 禁用旧版字段反向检查
...
### BEHAVIOR 4: error path — 文件存在且格式合法（YAML 含必要 key）

- [ ] [BEHAVIOR] `agent-e2e-video.yml` 文件存在且含 `workflow_dispatch`、`agent_version`、`default:` 三个关键词

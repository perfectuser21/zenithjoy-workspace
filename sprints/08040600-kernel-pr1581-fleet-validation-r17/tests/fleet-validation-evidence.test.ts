...
const repository = 'perfectuser21/zenithjoy-workspace';
const prNumber = 1581;
const baseSha = '676fed7de12023d355deac7849af8a525ae53f8d';
...
const roleEntry = new URL('../scripts/run-role-with-evidence.mjs', import.meta.url);
...
function roleFixture(role: string, marker: string): Json {
  return {
    ...
    attempt_id: dynamicId(marker), repository, pr_number: prNumber, base_sha: baseSha,
    final_sha: finalSha, provider: 'runner-provider',
...
function expectIdentity(value: Json, role: string): void {
  expect(value).toMatchObject({
    schema_version: 2, role, run_id: runId, repository, pr_number: prNumber,
    base_sha: baseSha, final_sha: finalSha,
  });
...
describe('PR #1581 fleet evidence chain [BEHAVIOR]', () => {
  ...
  it('Evaluator 保留失败证据并由 exit code 确定 verdict', async () => {
    ...
    expect(evaluator.upstream_provenance).toMatchObject({
      ...
      repository, pr_number: prNumber, base_sha: baseSha, final_sha: finalSha,
  ...
  it('Evaluator 生产入口拒绝旧 attempt 和失败 behavior', async () => {
    ...
    try {
      ...
      await writeJson(generatorPath, { ...generator, pr_number: 1580, behavior_tests: [behavior('generator-ok')] });
      await runRejected(evaluatorEntry, [
        '--generator', generatorPath, '--logical-cycle-id', 'cycle-current',
        '--upstream-attempt-id', generator.attempt_id,
        '--upstream-capability-snapshot-id', generator.capability_snapshot_id,
      ...
      const wrongPrResult = JSON.parse(await readFile(resultPath, 'utf8'));
      expect(wrongPrResult.verdict).toBe('FAIL');
      expect(wrongPrResult.behavior_tests).toContainEqual(expect.objectContaining({ name: 'target-pr-binding-rejected' }));
    } finally {
  ...
  it('Judge 生产入口拒绝旧 provenance、缺失证据和失败 behavior', async () => {
    ...
    try {
      ...
      await writeJson(evaluatorPath, { ...evaluator, repository: 'perfectuser21/other-repo', behavior_tests: [behavior('evaluator-ok')] });
      await runRejected(judgeEntry, [
        '--generator', generatorPath, '--evaluator', evaluatorPath,
        '--logical-cycle-id', 'cycle-current', '--upstream-attempt-id', evaluator.attempt_id,
        '--upstream-capability-snapshot-id', evaluator.capability_snapshot_id,
      ...
      const wrongPrResult = JSON.parse(await readFile(resultPath, 'utf8'));
      expect(wrongPrResult.verdict).toBe('INSUFFICIENT_EVIDENCE');
      expect(wrongPrResult.behavior_tests).toContainEqual(expect.objectContaining({ name: 'target-pr-binding-rejected' }));
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  ...
  it('中断与错误目标 PR 均不得记录成功', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pr1581-signal-negative-'));
    try {
      for (const [signal, expectedExit] of [['SIGINT', 130], ['SIGTERM', 143]] as const) {
        const resultPath = join(dir, `${signal}.json`);
        await runRejected(roleEntry, ['--self-test-signal', signal, '--result', resultPath]);
        const result = JSON.parse(await readFile(resultPath, 'utf8'));
        expect(result.exit_code).toBe(expectedExit);
        expect(result.verdict).not.toBe('PASS');
        expect(result.behavior_tests).toContainEqual(expect.objectContaining({
          name: 'interrupted-signal', exit_code: expectedExit,
        }));
        expect(result.finalizer_invocations).toBe(1);
      }
    } finally {
      await rm(dir, { recursive: true, force: true });
    }

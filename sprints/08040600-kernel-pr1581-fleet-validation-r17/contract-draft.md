...
本 Sprint 不新增业务 HTTP 响应。角色证据统一为 schema v2：顶层必含 `schema_version`、`role`、`run_id`、`attempt_id`、`final_sha`、`provider`、`account`、`machine`、`model`、`runner_digest`、`capability_snapshot_id`、`fleet_receipt_sha256`、`exit_code`、`log_tail`、`behavior_tests`；Evaluator/Judge 另含 `verdict` 和上游摘要。禁用：把 authoring attempt/snapshot UUID 写成期望值、缺失败条目的空 `behavior_tests`、用角色脚本生成 `fleet-receipt.json`。
...
1. Fleet 在启动每个角色命令前写出 checkout 外的 receipt，并注入 `HARNESS_FLEET_RECEIPT_PATH`。receipt 包含 `issued_by=fleet-runner`、`issued_before_role_start=true`、唯一 `receipt_id`、角色、run 以及该角色实际 `HARNESS_*`/`CAPABILITY_SNAPSHOT_ID`。
...
4. Generator、Evaluator、Judge 各用自己的 attempt/capability；Evaluator 引用 Generator 文件摘要，Judge 引用 Evaluator 文件摘要，禁止要求三者共用身份。
...
node "$SPRINT_DIR/scripts/verify-fleet-receipt.mjs" --path "$HARNESS_FLEET_RECEIPT_PATH" --role "$ROLE" --run-id "$RUN_ID" --started-at "$HARNESS_ROLE_STARTED_AT" 2>&1 | tee -a "$LOG" || exit 1
...
case "$ROLE" in
  ...
  evaluator) node "$SPRINT_DIR/scripts/evaluate-pr1581-evidence.mjs" --generator "$EVIDENCE_DIR/generator.json" --result "$RESULT" 2>&1 | tee -a "$LOG"; EXIT_CODE=${PIPESTATUS[0]} ;;
  judge) node "$SPRINT_DIR/scripts/judge-pr1581-evidence.mjs" --generator "$EVIDENCE_DIR/generator.json" --evaluator "$EVIDENCE_DIR/evaluator.json" --result "$RESULT" 2>&1 | tee -a "$LOG"; EXIT_CODE=${PIPESTATUS[0]} ;;

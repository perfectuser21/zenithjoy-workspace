...
每个角色在自己的证据中记录自己的 `HARNESS_ATTEMPT_ID`、`HARNESS_PROVIDER`、`HARNESS_ACCOUNT`、`HARNESS_MACHINE`、`HARNESS_MODEL`、`HARNESS_RUNNER_DIGEST` 与 `CAPABILITY_SNAPSHOT_ID`。三角色不得共用 attempt/account/snapshot：
...
**可观测行为**: attempt 空库完成真实 migration/signup/cookie 后，双租户 effective-config 合法更新、非法部分更新拒绝、并发后有效态成立；Generator receipt 绑定本角色 runtime identity。
...
**硬阈值**: product_validation.exit_code=0，总耗时 ≤7200s，final_sha 精确相等，Generator machine=us-mac-m4。

### Step 3: Evaluator 独立验证并引用 Generator receipt
**来源**: `[FROM_PRD]` — PRD「Golden Path」第 2-3 步。

**可观测行为**: Evaluator 用自己的 Runner identity 产生结构完整 evidence，PASS 时 `generator_receipt_sha256` 与实际 Generator receipt 摘要一致。
...
**硬阈值**: exit_code=0、behavior_tests 非空且完整、verdict=PASS、摘要精确相等。

### Step 4: Judge 基于 Evaluator evidence 给出最终 verdict
**来源**: `[FROM_PRD]` — PRD「Golden Path」第 3 步与边界情况。

**可观测行为**: Judge 使用自己的 Runner identity，以 Evaluator 文件 SHA-256 为输入；结果明确为 PASS/FAIL/INSUFFICIENT_EVIDENCE，并直接核验 verdict 前 PR 未合并。
...
**硬阈值**: Judge evidence 摘要引用一致；GitHub 状态 open、merged=false、merged_at=null；命令 exit 0。
...
> Evaluator 必须用自己的 Runner 注入身份生成 `evaluator.json`，引用 `generator.receipt.sha256`；Judge 必须用自己的 Runner 注入身份生成 `judge.json`，引用 Evaluator 文件 SHA-256。Evaluator/Judge 的 `attempt_id`、account、capability snapshot 不要求相同，也不得复制 Generator 值。最终从合同分支运行失败测试验摘要链和 GitHub 未合并状态。

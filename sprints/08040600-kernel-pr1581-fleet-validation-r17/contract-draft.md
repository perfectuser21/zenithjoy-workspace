...
- `npm run product-map:check` 已在锁定依赖安装后通过（digest `5011e4ef...`）。
...
每个角色还必须在自己的 Runner 进程内、执行角色工作前，把上述注入值按固定 key 集合写入 `evidence/<role>.runner-attestation.json`（`flag: wx`），并在角色 evidence 中写入该文件原始字节的 `runner_attestation_sha256`。后续角色与测试必须从 attestation 文件重新计算摘要，并逐字段比较 evidence；禁止只检查字段非空、只比较三角色互不相同或由 evidence 反向生成 attestation。这样角色身份同时受 Runner 注入值、独立 attestation 原始字节和摘要链机械约束。
...
| 风险 | 触发信号 | 阻断/缓解 | 剩余风险 |
|---|---|---|---|
| Runner 环境被角色进程内恶意篡改 | attestation 与 evidence 任一字段或摘要不一致 | attestation 在角色工作前以 `wx` 写入；测试逐字段及 SHA-256 核验 | 当前合同依赖 Fleet/Runner 正确注入，无法替代平台签名证明 |
| PR head 在长验证期间漂移或提前合并 | 前后 GitHub API 结果不同，或 merged=true | 立即失败，禁止沿用证据；新 SHA/new attempt 全量重跑 | 两次查询之间仍存在极短竞态窗口 |
| us-mac-m4 或第三方 GitHub API 不可用 | machine 不符、请求超时/非零 | fail-closed，不 fallback、不把缺证据判 PASS | 外部故障会导致 INSUFFICIENT_EVIDENCE，而非产品结论 |
| 摘要链文件被覆盖或重放 | `wx` 失败、摘要不匹配、角色 attempt/snapshot 重复 | 每角色新 attempt、新文件、两两不同与逐级 SHA-256 引用 | 工作区本身无防篡改存储，最终报告须保留原始 receipt |
...
**可观测行为**: attempt 空库完成真实 migration/signup/cookie 后，双租户 effective-config 合法更新、非法部分更新拒绝；同一租户的两条冲突 PATCH 真实并发发出，仅一条成功、另一条返回 `INVALID_CONFIG`，最终 `min <= max`；Generator receipt 绑定本角色 runtime identity 与 Runner attestation SHA-256。
...
**可观测行为**: Evaluator 用自己的 Runner identity 产生结构完整 evidence，逐字段匹配其独立 Runner attestation；PASS 时 `generator_receipt_sha256` 与实际 Generator receipt 摘要一致，且其 attempt/capability snapshot 均与 Generator 不同。
...
**可观测行为**: Judge 使用自己的 Runner identity 并逐字段匹配其独立 Runner attestation，以 Evaluator 文件 SHA-256 为输入；三角色 attempt/capability snapshot 分别两两不相等；结果明确为 PASS/FAIL/INSUFFICIENT_EVIDENCE，并直接核验 verdict 前 PR 未合并。
...
node - "$OLDPWD/$SPRINT_DIR/evidence/generator.runner-attestation.json" <<'NODE'
const fs=require('fs');
...
const a={schema_version:1,issued_by:'fleet-runner',role:'generator',attempt_id:process.env.HARNESS_ATTEMPT_ID,provider:process.env.HARNESS_PROVIDER,account:process.env.HARNESS_ACCOUNT,machine:process.env.HARNESS_MACHINE,model:process.env.HARNESS_MODEL,runner_digest:process.env.HARNESS_RUNNER_DIGEST,capability_snapshot_id:process.env.CAPABILITY_SNAPSHOT_ID};
...
const attestation=fs.readFileSync(out.replace(/generator\.json$/,'generator.runner-attestation.json'));
const data={schema_version:1,role:'generator',run_id:'5172f36e-d86c-45cf-a417-b2678c2ec3e4',attempt_id:process.env.HARNESS_ATTEMPT_ID,repository:'perfectuser21/zenithjoy-workspace',pr_number:1581,base_sha:'676fed7de12023d355deac7849af8a525ae53f8d',final_sha:'c305f6217da65bb69413c39e621b7e797e0fb189',provider:process.env.HARNESS_PROVIDER,account:process.env.HARNESS_ACCOUNT,machine:process.env.HARNESS_MACHINE,model:process.env.HARNESS_MODEL,runner_digest:process.env.HARNESS_RUNNER_DIGEST,capability_snapshot_id:process.env.CAPABILITY_SNAPSHOT_ID,runner_attestation_sha256:crypto.createHash('sha256').update(attestation).digest('hex'),started_at:process.env.STARTED_AT,finished_at:process.env.FINISHED_AT,product_validation:{exit_code:0,log_tail:'exact-SHA real-signup two-tenant concurrent effective-config validation passed',concurrent_effective_config:{response_statuses:statuses,final_min:Number(process.env.CONCURRENT_MIN),final_max:Number(process.env.CONCURRENT_MAX),invalid_error_code:'INVALID_CONFIG'}}};
...
> Evaluator/Judge 必须分别先以自身 Runner 注入值和 `flag: wx` 写 `evaluator.runner-attestation.json`/`judge.runner-attestation.json`，再生成对应 evidence；evidence 的 `runner_attestation_sha256` 必须引用 attestation 原始字节。Evaluator 另引用 `generator.receipt.sha256`；Judge 另引用 Evaluator 文件 SHA-256。三角色的 `attempt_id` 与 `capability_snapshot_id` 必须分别两两不相等；account 如实记录各 Runner 值而不强制不同。最终从合同分支运行失败测试逐字段核验三份 attestation、摘要链和 GitHub 未合并状态。

...
- Round 7 修复 reviewer 阻塞：Evaluator 明确扫描 Generator 顶层及每条 behavior；Judge 明确扫描 Generator/Evaluator 顶层及每条 behavior。测试分别注入上游 behavior 非零，确定性 oracle 必须返回 FAIL，堵住局部成功覆盖失败。
- Round 8 修复 reviewer 阻塞：负向验收不再只调用测试内纯判定函数；Vitest 必须以临时 evidence 真实启动 Evaluator/Judge 生产 CLI，证明旧 attempt、缺失 evidence、失败 behavior 会产生非零进程退出与落盘拒绝 verdict。
- Round 9 修复 reviewer 阻塞：Judge 负向验收新增同 run/SHA 但旧 logical cycle/attempt/capability 的 Evaluator evidence，真实启动 Judge 生产 CLI，必须非零退出并落盘 `INSUFFICIENT_EVIDENCE` 与 `old-attempt-replay-rejected`。
- Round 10 修复 reviewer 阻塞：INT/TERM 固定为 130/143 后进入唯一 EXIT finalizer，禁止中断被记为成功；三角色 evidence 及 upstream provenance 均逐字段绑定目标仓库、PR、base SHA 与 final SHA。

## Response Schema（推导来源: PRD 字面 + NEW_PATTERN）

本 Sprint 不新增业务 HTTP 响应。角色证据统一为 schema v2：顶层必含 `schema_version`、`role`、`run_id`、`logical_cycle_id`、`attempt_id`、`repository`、`pr_number`、`base_sha`、`final_sha`、`provider`、`account`、`machine`、`model`、`runner_digest`、`capability_snapshot_id`、`fleet_receipt_sha256`、`exit_code`、`log_tail`、`behavior_tests`；Evaluator/Judge 另含 `verdict`、上游摘要与 `upstream_provenance`，且 provenance 重复携带同一组 PR 四元组。禁用：把 authoring attempt/snapshot UUID 写成期望值、只凭 run/SHA 接受上游证据、缺失败条目的空 `behavior_tests`、用角色脚本生成 `fleet-receipt.json`。
...
6. 每份角色 evidence 与每个 `upstream_provenance` 必须逐字段等于 `repository=perfectuser21/zenithjoy-workspace`、`pr_number=1581`、冻结 base SHA 与目标 final SHA。任一字段不符时 Evaluator=`FAIL`、Judge=`INSUFFICIENT_EVIDENCE`，均写 `target-pr-binding-rejected` 并非零退出。
7. INT/TERM 不得走普通成功路径：signal handler 先写 `interrupted-signal` 失败 behavior，并固定 INT=130、TERM=143，再进入唯一 EXIT finalizer。
...
**可观测行为**: Evaluator 用自身前置 receipt，引用 Generator 摘要；每项断言保留实际 exit code/log_tail/evidence。`verdict=PASS` 当且仅当 Generator 顶层、Generator 全部 behavior、Evaluator 顶层和 Evaluator 全部 behavior 的 exit code 均为 0；任一非零都必须为 `FAIL`。
...
Evaluator 自身 envelope 与 Generator provenance 还必须逐字段绑定目标 repository、PR #1581、base SHA 与 final SHA；PR number 错误必须非零退出、`verdict=FAIL` 并写 `target-pr-binding-rejected`。

**验证命令**: `npx vitest run sprints/08040600-kernel-pr1581-fleet-validation-r17/tests/fleet-validation-evidence.test.ts -t "Evaluator 生产入口拒绝旧 attempt 和失败 behavior" --reporter=verbose`

**硬阈值**: 测试用同 run/SHA 但 provenance 不符的 Generator evidence 真实启动 Evaluator CLI，进程非零且 result 落盘 `verdict=FAIL`、`old-attempt-replay-rejected`；provenance 正确但 behavior exit_code=9 时同样非零且 `verdict=FAIL`；命令 exit 0。

### Step 4: Judge 产生唯一一致的最终 verdict
**来源**: `[FROM_PRD]` — PRD「Golden Path」第 3 步及边界情况。

**可观测行为**: Judge 用自己的前置 receipt 并引用 Evaluator 文件摘要。上游证据缺失/不可解析=`INSUFFICIENT_EVIDENCE`；Generator 或 Evaluator 的顶层/任一 behavior 非零，或 Evaluator verdict 非 PASS，均为 `FAIL`；证据齐全且两角色顶层与全部 behavior 全绿才可 `PASS`。Judge 自身每项核验也保留实际结果。
...
Judge 自身 envelope、Generator 与 Evaluator evidence/provenance 均须逐字段绑定同一目标 PR 四元组；repository 错误必须非零退出、`verdict=INSUFFICIENT_EVIDENCE` 并写 `target-pr-binding-rejected`。
...
**硬阈值**: 真实启动 Judge CLI：旧 Evaluator provenance 时进程非零且 result 为 `INSUFFICIENT_EVIDENCE`并含 `old-attempt-replay-rejected`；Evaluator 文件缺失时进程非零且 result 为 `INSUFFICIENT_EVIDENCE`；Evaluator behavior exit_code=9 时进程非零且 result 为 `FAIL`；三次均落盘非零 `behavior_tests`；PR 未合并；命令 exit 0。
...
| Evaluator 负向生产入口 | `tests/fleet-validation-evidence.test.ts` | Evaluator 生产入口拒绝旧 attempt 和失败 behavior | CLI/helper 尚未实现，进程启动失败 |
| Judge 负向生产入口 | `tests/fleet-validation-evidence.test.ts` | Judge 生产入口拒绝旧 provenance、缺失证据和失败 behavior | CLI/helper 尚未实现，进程启动失败 |
| 信号与 PR 绑定 | `tests/fleet-validation-evidence.test.ts` | 中断与错误目标 PR 均不得记录成功 | signal/target binding helper 尚未实现 |
...
EXIT_CODE=1
finalize(){ rc=$?; trap - EXIT INT TERM; [ "$EXIT_CODE" -ne 1 ] || EXIT_CODE=$rc; node "$SPRINT_DIR/scripts/finalize-role-evidence.mjs" --role "$ROLE" --exit-code "$EXIT_CODE" --log "$LOG" --result "$RESULT" --fleet-receipt "$HARNESS_FLEET_RECEIPT_PATH" --output "$EVIDENCE_DIR/$ROLE.json"; exit "$EXIT_CODE"; }
on_signal(){ EXIT_CODE="$1"; printf 'interrupted-signal exit_code=%s\n' "$EXIT_CODE" | tee -a "$LOG"; node "$SPRINT_DIR/scripts/record-failed-behavior.mjs" --result "$RESULT" --name interrupted-signal --exit-code "$EXIT_CODE" --log "$LOG"; exit "$EXIT_CODE"; }
trap finalize EXIT

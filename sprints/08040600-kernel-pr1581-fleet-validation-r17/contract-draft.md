...
**验证命令**: `PR=$(gh api repos/perfectuser21/zenithjoy-workspace/pulls/1581) && echo "$PR" | jq -e '.state=="open" and .merged==false and .merged_at==null and .head.sha=="c305f6217da65bb69413c39e621b7e797e0fb189"' && npx vitest run sprints/08040600-kernel-pr1581-fleet-validation-r17/tests/fleet-validation-evidence.test.ts -t "Judge 生产入口拒绝缺失证据和失败 behavior" --reporter=verbose`

**硬阈值**: 真实启动 Judge CLI：Evaluator 文件缺失时进程非零且 result 为 `INSUFFICIENT_EVIDENCE`；Evaluator behavior exit_code=9 时进程非零且 result 为 `FAIL`；两次均落盘非零 `behavior_tests`；PR 未合并；命令 exit 0。
...
| Judge 负向生产入口 | `tests/fleet-validation-evidence.test.ts` | Judge 生产入口拒绝缺失证据和失败 behavior | CLI/helper 尚未实现，进程启动失败 |

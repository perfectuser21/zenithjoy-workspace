...
**硬阈值**: envelope 完整、摘要一致；将 Generator 任一 behavior 的 exit_code 改为 9 后确定性 oracle 必须得 `FAIL`；真实证据 verdict 与同一规则一致；命令 exit 0。
...
**硬阈值**: verdict 属三值且与规则唯一一致；分别将 Generator、Evaluator 任一 behavior 的 exit_code 改为 9，确定性 oracle 均必须得 `FAIL`；PR 未合并；命令 exit 0。

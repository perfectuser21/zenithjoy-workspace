...
- [ ] [BEHAVIOR] [L2] B-02: Evaluator 保留失败证据并由结果确定 verdict [接缝×2]
  ...
  预期观察: logical_cycle_id 及 Runner 注入的 Generator attempt/capability/SHA-256 provenance 全部精确匹配；顶层和全部 behavior exit 0 时 verdict=PASS，否则 verdict=FAIL；失败条目不被丢弃或改写为成功
  等待预算: 600s
...
- [ ] [BEHAVIOR] [L2] B-03: Judge 摘要串联并输出唯一一致裁决 [接缝×2]
  ...
  预期观察: logical_cycle_id 及 Runner 注入的 Evaluator attempt/capability/SHA-256 provenance 全部精确匹配；缺证或 provenance 不符为 INSUFFICIENT_EVIDENCE；完整证据任一失败为 FAIL；证据齐全且全部成功才 PASS；PR 仍 open、未合并且 head 精确匹配
  等待预算: 600s

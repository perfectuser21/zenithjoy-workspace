...
- [ ] [ARTIFACT] `evidence/generator.json`、`generator.receipt.sha256`、`evaluator.json`、`judge.json` 均由对应角色新生成并保留原始 log_tail/evidence。
  Test: node -e "for(const f of ['generator.json','evaluator.json','judge.json'])JSON.parse(require('fs').readFileSync('sprints/08040600-kernel-pr1581-fleet-validation-r17/evidence/'+f,'utf8'));require('fs').accessSync('sprints/08040600-kernel-pr1581-fleet-validation-r17/evidence/generator.receipt.sha256')"
...
- [ ] [BEHAVIOR] [L2] B-01: Generator 新鲜证据绑定运行时身份和目标 SHA [接缝×2]
  ...
  预期观察: Generator 证据记录自身 Runner identity，真实并发响应状态为 200/400、失败码为 INVALID_CONFIG、最终 min<=max，且目标 SHA 和实际机器精确匹配
  等待预算: 7200s
...
- [ ] [BEHAVIOR] [L2] B-02: Evaluator 使用自己的身份并引用 Generator receipt 摘要 [接缝×2]
  ...
  预期观察: verdict=PASS、结构完整，generator_receipt_sha256 等于真实 receipt，且 Evaluator 与 Generator 的 attempt/capability snapshot 分别不相等
  等待预算: 600s
...
- [ ] [BEHAVIOR] [L2] B-03: Judge 使用自己的身份引用 Evaluator 摘要且裁决前未合并 [接缝×2]
  ...
  预期观察: evaluator_evidence_sha256 精确匹配；三角色 attempt/capability snapshot 各自两两不相等；最终 verdict 明确；PR 仍 open、未合并且 head 未漂移
  等待预算: 600s

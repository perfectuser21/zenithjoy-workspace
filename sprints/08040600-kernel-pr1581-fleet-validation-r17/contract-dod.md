...
- [ ] [BEHAVIOR] [L2] B-02: Evaluator 保留失败证据并由结果确定 verdict [接缝×2]
  ...
  预期观察: logical_cycle_id 及 Runner 注入的 Generator attempt/capability/SHA-256 provenance 全部精确匹配；Generator 与 Evaluator 顶层和全部 behavior exit 0 时 verdict=PASS，任一 Generator/Evaluator behavior 非零时 verdict=FAIL；失败条目不被丢弃或改写为成功
  等待预算: 600s
...
- [ ] [BEHAVIOR] [L2] B-03: Judge 摘要串联并输出唯一一致裁决 [接缝×2]
  ...
  预期观察: logical_cycle_id 及 Runner 注入的 Evaluator attempt/capability/SHA-256 provenance 全部精确匹配；缺证或 provenance 不符为 INSUFFICIENT_EVIDENCE；Generator 或 Evaluator 任一 behavior 非零为 FAIL；证据齐全且两个上游角色顶层及全部 behavior 成功才 PASS；PR 仍 open、未合并且 head 精确匹配
  等待预算: 600s
...
- [ ] [BEHAVIOR] [L2] B-05: 同 run 和 SHA 的旧 attempt 重放必须被 provenance 拒绝
  动作: 分别向 Evaluator 与 Judge 输入 run_id、final_sha 均正确但 logical_cycle_id、attempt_id、capability_snapshot_id 不等于 Runner receipt 上游 provenance 的旧 evidence
  预期观察: 两个角色均拒绝；Evaluator 非零退出，Judge 为 INSUFFICIENT_EVIDENCE；各自 behavior_tests 含 old-attempt-replay-rejected 且不得 PASS
  等待预算: 30s
  留证: Vitest stdout 与 evaluator/judge evidence 中的 replay rejection behavior
  Test: manual:bash -c 'npx vitest run sprints/08040600-kernel-pr1581-fleet-validation-r17/tests/fleet-validation-evidence.test.ts -t "同 run 和 SHA 的旧 attempt 重放必须被 provenance 拒绝" --reporter=verbose'

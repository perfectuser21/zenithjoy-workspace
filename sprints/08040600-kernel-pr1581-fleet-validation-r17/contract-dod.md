...
- [ ] [BEHAVIOR] [L2] B-01: Generator 新鲜证据绑定运行时身份和目标 SHA [接缝×2]
  ...
  预期观察: Generator 证据记录自身 Runner identity，product_validation.exit_code=0，目标 SHA 和实际机器精确匹配
  等待预算: 7200s
  留证: evidence/generator.json、generator.receipt.sha256 与产品 log_tail
...
- [ ] [BEHAVIOR] [L2] B-02: Evaluator 使用自己的身份并引用 Generator receipt 摘要 [接缝×2]
  ...
  预期观察: verdict=PASS、结构完整，generator_receipt_sha256 等于真实 receipt 且 Evaluator attempt/capability 非空
  等待预算: 600s
...
- [ ] [BEHAVIOR] [L2] B-03: Judge 使用自己的身份引用 Evaluator 摘要且裁决前未合并 [接缝×2]
  ...
  预期观察: evaluator_evidence_sha256 精确匹配；最终 verdict 明确；PR 仍 open、未合并且 head 未漂移
  等待预算: 600s

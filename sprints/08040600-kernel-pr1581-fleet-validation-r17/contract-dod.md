...
- [ ] [BEHAVIOR] [L2] B-03: Judge 摘要串联并输出唯一一致裁决 [接缝×2]
  ...
  预期观察: 旧 provenance 调用非零退出并落盘 INSUFFICIENT_EVIDENCE + old-attempt-replay-rejected；缺证调用非零退出并落盘 INSUFFICIENT_EVIDENCE；失败 behavior 调用非零退出并落盘 FAIL；三者均含非零 behavior；只有 provenance 匹配且上游全部成功才 PASS；PR 仍 open、未合并且 head 精确匹配
  等待预算: 600s

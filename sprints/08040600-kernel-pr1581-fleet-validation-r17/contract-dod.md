...
- [ ] [BEHAVIOR] [L2] B-02: Evaluator 保留失败证据并由结果确定 verdict [接缝×2]
  ...
  预期观察: 两个负向调用均非零退出并落盘 verdict=FAIL；旧 attempt 结果含 old-attempt-replay-rejected；失败 behavior 不被丢弃；本轮证据仅在 provenance 精确匹配且全部 behavior exit 0 时 PASS
  等待预算: 600s
...
- [ ] [BEHAVIOR] [L2] B-03: Judge 摘要串联并输出唯一一致裁决 [接缝×2]
  动作: 以临时 evidence 分别删除 Evaluator 文件与注入 behavior exit_code=9，真实启动 Judge 生产 CLI；随后核验本轮 Judge evidence、Evaluator SHA-256 与真实 GitHub PR
  预期观察: 缺证调用非零退出并落盘 INSUFFICIENT_EVIDENCE；失败 behavior 调用非零退出并落盘 FAIL；两者均含非零 behavior；只有 provenance 匹配且上游全部成功才 PASS；PR 仍 open、未合并且 head 精确匹配
  等待预算: 600s
...
- [ ] [BEHAVIOR] [L2] B-05: 同 run 和 SHA 的旧 attempt 重放必须被 provenance 拒绝
  动作: 向 Evaluator 生产 CLI 输入 run_id、final_sha 正确但 provenance 过期的 evidence；向 Judge 生产 CLI 输入缺失 Evaluator evidence
  预期观察: 两个真实进程均非零退出并落盘拒绝结果；Evaluator 为 FAIL 且含 old-attempt-replay-rejected，Judge 为 INSUFFICIENT_EVIDENCE 且含缺证失败 behavior；均不得 PASS
  等待预算: 30s
  留证: Vitest stdout 与 evaluator/judge evidence 中的 replay rejection behavior
  Test: manual:bash -c 'npx vitest run sprints/08040600-kernel-pr1581-fleet-validation-r17/tests/fleet-validation-evidence.test.ts -t "Evaluator 生产入口拒绝旧 attempt 和失败 behavior|Judge 生产入口拒绝缺失证据和失败 behavior" --reporter=verbose'

...
- **（round 3 修订，修复合同鉴权假设错误——GAN 未审出的真实功能 bug，见 PR fix-realmachine-apk-fetch-401 / task 1d087bfe-cf40-4d28-a5b4-76383565510e）** `GET /api/agent/install-pack/android` 是给浏览器登录客户用的 better-auth session cookie 鉴权端点（`apps/api/src/routes/agent-install-pack.ts:360-372`，`auth.api.getSession()` 未登录返回 401；`android-onboarding-smoke.sh:7-10` 本身就明确断言"无 session → 401"是设计行为）。CI self-hosted runner（xian-rog）没有浏览器 session，round 1/2 把"该端点已被 android-onboarding-smoke.sh 验证过"误当成"真机 smoke 也能直接调用"的依据——这是错的，两者鉴权前提不同。真机 smoke 的"安装最新 APK"步骤改为直连该端点内部同样使用的公网 COS 直链常量（`apps/api/src/routes/agent-install-pack.ts:390-393`，`ANDROID_APK_COS_URL` 环境变量同款覆盖约定），不经过需要浏览器 session 的端点。

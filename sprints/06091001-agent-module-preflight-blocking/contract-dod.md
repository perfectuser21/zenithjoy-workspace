...
**范围**: preflight.py 四层锁扩展 + start.bat blocking + /module-health 权限开放 + Line04PreflightCard 组件
**大小**: M
...
- [ ] [ARTIFACT] `services/agent/wechat-rpa/preflight.py` `check_lock_update()` 含四层锁实现
...
- [ ] [ARTIFACT] `services/agent/install-pack/start.bat` preflight 段改为 blocking（含 exit /b 1）
...
- [ ] [ARTIFACT] `.github/workflows/agent-preflight-hardening-e2e.yml` 新建（windows-latest，含 E2E-1~5）
  Test: node -e "require('fs').accessSync('.github/workflows/agent-preflight-hardening-e2e.yml');console.log('OK')"
...
**自查**: 若 preflight.py 未实现 9 项（或 --dry-run 写 JSON 失败），jq 断言 → FAIL ✅
...
### [BEHAVIOR 2] check_lock_update 四层锁：icacls DENY

**Golden Path 对应**: Step 2 — check_lock_update 执行四层锁
**自查**: 若 Generator 未加 icacls 调用，icacls 输出无 DENY → FAIL ✅
...
### [BEHAVIOR 3] check_lock_update 四层锁：dldir1v6.qq.com 防火墙 + AutoUpdate=0 注册表

**Golden Path 对应**: Step 2 — 四层锁 Layer 3/4
**自查**: 若未加防火墙规则和注册表写入，断言 → FAIL ✅
...
assert \"dldir1v6.qq.com\" in fw.stdout, f\"FAIL Layer3: 防火墙无 dldir1v6 规则\"
...
**自查**: 若 Generator 未改 start.bat blocking，旧代码 exit 0 继续，proc.ExitCode==0 → FAIL ✅（或旧文本仍存在）
...
7. **无 MOCK_* 环境变量注入**: PREFLIGHT_MOCK_FAIL=1 是 start.bat E2E-3 的 CI stub（来自 PRD），非绕过真实检测 ✅

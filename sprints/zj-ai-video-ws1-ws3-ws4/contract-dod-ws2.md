...
**范围**: 前端比例选择器 + createJob 携带 target_aspect；Agent ffprobe 读 width/height + rotation swap → detectedAspect → PATCH 写回；effectiveTarget 只生成单文件
**大小**: M（~120 行净增，2 文件）
...
- [ ] [ARTIFACT] Agent video-pipeline.ts 含 detectedAspect 变量声明 + effectiveTarget 变量声明
...
- [ ] [ARTIFACT] Agent video-pipeline.ts 含 PATCH detected_aspect 写回调用（不能只算不写）
...
- [ ] [BEHAVIOR] Agent handler effectiveTarget 逻辑存在：target_aspect ?? detectedAspect ?? "9:16"，且只生成单一输出文件
  Test: manual:bash -c '
    node -e "
      const c = require(\"fs\").readFileSync(\"services/agent/src/handlers/video-pipeline.ts\",\"utf8\");
      if (!c.includes(\"effectiveTarget\")) process.exit(1);
      // 验证 target_aspect ?? detectedAspect 模式存在
...
- [ ] [BEHAVIOR] PATCH /api/ai-video/jobs/:id/progress 接受 detected_aspect 并写库（DB 验证）
  Test: manual:bash -c '
    TEST_JOB_ID=$(psql "$DB" -t -c "
      ...
      RETURNING id
    " | tr -d " \n")
    curl -sf -X PATCH "localhost:3000/api/ai-video/jobs/$TEST_JOB_ID/progress" \
      -H "Content-Type: application/json" \
      -d "{\"progress\":35,\"status\":\"processing\",\"detected_aspect\":\"9:16\"}" || { echo "FAIL: PATCH 请求失败"; exit 1; }
    RESULT=$(psql "$DB" -t -c "SELECT detected_aspect FROM zenithjoy.ai_video_pipeline_jobs WHERE id='"'"'$TEST_JOB_ID'"'"'" | tr -d " \n")
...
- [ ] [BEHAVIOR] PATCH 响应禁用字段 aspectRatio/aspect/ratio/orientation 不出现
  Test: manual:bash -c '
    TEST_JOB_ID=$(psql "$DB" -t -c "
      INSERT INTO zenithjoy.ai_video_pipeline_jobs (src_video, status, progress) VALUES ('\''C:\\test.mp4'\'', '\''processing'\'', 10) RETURNING id
    " | tr -d " \n")
    RESP=$(curl -sf -X PATCH "localhost:3000/api/ai-video/jobs/$TEST_JOB_ID/progress" \
      -H "Content-Type: application/json" \

...
- [ ] [ARTIFACT] 新建三层记忆 migration（zenithjoy schema，per tenant_id × contact，幂等 IF NOT EXISTS）
...
- [ ] [ARTIFACT] 三层记忆服务文件存在且暴露写/取上下文/收尾三能力
...
- [ ] [ARTIFACT] 三个后端能力 route 已挂载到 app（/api/wechat/memory/*）
...
- [ ] [BEHAVIOR] Step1 写消息进短期：POST /message(租户A) → 200 ok + cs_memory_messages 5 分钟内新增（带时间窗防造假）
...
- [ ] [BEHAVIOR] Step4 取回复上下文三层拼接 + schema keys 完整性（context==[longterm,mid,short]）+ 禁用字段反向
...
- [ ] [BEHAVIOR] Step2/3 日收尾生成中期 + 跨天并入长期 + 空天不写空中期（DB 时间窗断言）
...
- [ ] [BEHAVIOR] Step5 隔离：租户A查只见A，B查只见B，DB 层跨租户泄漏 count=0（双向）
...
- [ ] [BEHAVIOR] Step6 error path：写/查缺 tenant_id 三端点均 400 + error=="MISSING_TENANT"，不回退不串租户
...
- [ ] [BEHAVIOR] 降级：summarization（OpenRouter）失败时不破坏已有三层数据，收尾链路降级仍写非空中期
...
- [ ] [BEHAVIOR] HTTP final-e2e：起真 apps/api，curl|jq 验三层 keys + 隔离 + 缺 tenant 400，psql 带 5 分钟时间窗断言落库

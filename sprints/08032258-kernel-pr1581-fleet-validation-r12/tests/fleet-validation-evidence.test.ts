...
const proposerAttemptId = '47cae3e5-e791-44d4-8b1c-0ac5ed9ba57b';
const proposerSnapshotId = 'b21f0c13-df1c-4235-a0d9-d53df2cb3ec2';
...
function fleetAdmissionAnchor(): { attemptId: string; startedAt: string; startedMs: number } {
  const attemptId = process.env.FLEET_ADMISSION_ATTEMPT_ID;
  const startedAt = process.env.FLEET_ADMISSION_STARTED_AT;
  expect(attemptId).toMatch(/^[0-9a-f]{8}-[0-9a-f-]{27}$/);
  const startedMs = expectValidTimestamp(startedAt);
  return { attemptId: attemptId as string, startedAt: startedAt as string, startedMs };
}
...
describe('PR #1581 真实 fleet 双裁决证据 [BEHAVIOR]', () => {
  it('唯一身份 SSOT 锁定验证 attempt 和执行面', async () => {
    const admission = fleetAdmissionAnchor();
    ...
    expect(identity.validation_attempt_id).toBe(admission.attemptId);
    ...
    expect(identity.created_at).toBe(admission.startedAt);
    expect(expectValidTimestamp(run.started_at)).toBeGreaterThanOrEqual(admission.startedMs);
    expectBoundToIdentity(run, identity);
    expect(run.capability_snapshot_id).toBe(identity.capability_snapshot_id);
  });

  it('Fleet 控制面外部锚点拒绝历史整包重放', async () => {
    const admission = fleetAdmissionAnchor();
    const [identity, run, evaluator, judge, gate] = await Promise.all([
    ...
    expect(identity.validation_attempt_id).toBe(admission.attemptId);
    expect(identity.created_at).toBe(admission.startedAt);
    for (const item of [run, evaluator, judge, gate]) expectBoundToIdentity(item, identity);
    expect(expectValidTimestamp(run.started_at)).toBeGreaterThanOrEqual(admission.startedMs);
  });

...
- 身份 SSOT 只有 `evidence/validation-identity.json`。Fleet 控制面在本次 admission 创建时生成只读外部锚点 `FLEET_ADMISSION_ATTEMPT_ID` 与 `FLEET_ADMISSION_STARTED_AT`，identity 必须逐字复制这两个值；fleet-run、Evaluator、Judge 与 merge gate 只允许引用该 identity，不得自行声明 attempt/time。
- 冻结 PRD 要求的验证执行面是 snapshot `cc3550be-875a-48d3-9be4-24343fb355a9`；本 proposer 角色自身的 attempt `47cae3e5-e791-44d4-8b1c-0ac5ed9ba57b` 与 capability snapshot `b21f0c13-df1c-4235-a0d9-d53df2cb3ec2` 仅是合同起草来源元数据，明确禁止写入 validation identity 或冒充产品验证执行面。
- Round 6 按 Reviewer 反馈把非法更新与并发更新全部改为目标 SHA 实际注册的 `PUT /api/acquisition/config`；E2E 不再调用未注册的更新方法，因此路由缺失会直接导致 `curl -f`/状态码断言失败。
...
Fleet 控制面必须在 admission 边界注入 `FLEET_ADMISSION_ATTEMPT_ID`（当前 admission 的 UUID）与 `FLEET_ADMISSION_STARTED_AT`（当前 admission 创建时的 UTC ISO-8601）；二者来自 evidence 目录之外的控制面调度上下文，缺失即阻塞，证据生成器不得赋值或回填。Fleet wrapper 在任何产品命令前原子创建 `evidence/validation-identity.json`；文件创建后只读，不得由 Evaluator、Judge 或 merge gate 覆盖。唯一合法结构为：

```json
{
  "schema_version": 1,
  "logical_run_id": "bfaf1e49-a8cb-401e-9fc3-d6c62c457edc",
  "validation_attempt_id": "<逐字等于 FLEET_ADMISSION_ATTEMPT_ID>",
  ...
  "created_at": "<逐字等于 FLEET_ADMISSION_STARTED_AT>"
}
```

`fleet-run.json`、`evaluator.json`、`independent-judge.json`、`merge-gate.json` 的 `run_id`/`attempt_id`/`final_sha` 必须分别等于此文件的 `logical_run_id`/`validation_attempt_id`/`final_sha`，且 `fleet-run.started_at >= FLEET_ADMISSION_STARTED_AT`。验收测试直接读取控制面环境变量做外部对账；因此同一 logical_run/final_sha 的历史五文件整包重放也必因 attempt 或 started_at 不同而失败。当前 proposer capability evidence 只证明合同在哪里起草，不参与上述验证身份。
...
| ⚠️ 双裁决是否新鲜且独立 | A. 看结论文本；B. 与控制面 admission attempt_id/started_at 外部锚点校验，再验 SHA/evidence_id/结构 | B. 外部锚点加全字段校验 | 冻结 PRD 明确禁止复用 | 旧证据或单一裁决假绿 |
...
- acquisition PUT 路由 ↔ service ↔ Postgres：必须在 attempt 空库迁移后启动真实 API，以 signup 产生的真实 session cookie 调用，禁止 mock 路由、service、pool 或 transaction client。
- Fleet 控制面 admission ↔ validation-identity ↔ runner execution evidence：控制面注入当前 admission attempt_id/started_at，identity 只能逐字复制，后续角色不得改写；必须来自要求 snapshot 的 `us-mac-m4` 实际运行记录。
...
**可观测行为**: merge gate 精确引用本轮实际 Evaluator/Judge evidence_id，只标记 eligible_for_human_confirmation=true，同时 human_confirmation_required=true、merge_performed=false；并直接查询 GitHub PR API，确认 PR 仍为 open、merged=false、merged_at=null 且 head SHA 未漂移，禁止只信 gate 自报字段。

**验证命令**: `PR=$(gh api repos/perfectuser21/zenithjoy-workspace/pulls/1581) && echo "$PR" | jq -e '.state=="open" and .merged==false and .merged_at==null and .head.sha=="c305f6217da65bb69413c39e621b7e797e0fb189"' && npx vitest run sprints/08032258-kernel-pr1581-fleet-validation-r12/tests/fleet-validation-evidence.test.ts -t '双裁决齐备后仍只开放人工确认而未自动合并' --reporter=verbose`

**硬阈值**: 命令 exit 0；GitHub 实际状态必须 open 且未合并；不得出现自动 merge 副作用。
...
|---|---|---|---|
| 验证身份 SSOT | `tests/fleet-validation-evidence.test.ts` | 唯一身份 SSOT 锁定验证 attempt 和执行面 | 控制面锚点或 evidence 缺失而失败 |
| 防历史重放 | `tests/fleet-validation-evidence.test.ts` | Fleet 控制面外部锚点拒绝历史整包重放 | 历史 attempt/time 与当前 admission 锚点不等 |
...
: "${FLEET_ADMISSION_ATTEMPT_ID:?Fleet control plane must inject current admission attempt UUID}"
: "${FLEET_ADMISSION_STARTED_AT:?Fleet control plane must inject current admission start time}"
[[ "$FLEET_ADMISSION_ATTEMPT_ID" =~ ^[0-9a-f]{8}-[0-9a-f-]{27}$ ]]
node -e 'if(!Number.isFinite(Date.parse(process.env.FLEET_ADMISSION_STARTED_AT)))process.exit(1)'
...
CODE=$(curl -sS -b "$COOKIE_A" -o /tmp/r12-invalid.json -w '%{http_code}' -X PUT -H 'content-type: application/json' -d '{"keywords_per_round_max":2}' "$BASE_URL/api/acquisition/config")
[ "$CODE" = 400 ]
...
curl -sS -b "$COOKIE_A" -o /tmp/r12-concurrent-min.json -w '%{http_code}' -X PUT -H 'content-type: application/json' -d '{"keywords_per_round_min":9}' "$BASE_URL/api/acquisition/config" >/tmp/r12-code-min &
P1=$!
curl -sS -b "$COOKIE_A" -o /tmp/r12-concurrent-max.json -w '%{http_code}' -X PUT -H 'content-type: application/json' -d '{"keywords_per_round_max":8}' "$BASE_URL/api/acquisition/config" >/tmp/r12-code-max &
P2=$!
wait "$P1"
wait "$P2"
...
> 后续收证顺序（不可并行伪造）：Fleet 控制面先在 evidence 目录外生成并注入当前 admission 的 `FLEET_ADMISSION_ATTEMPT_ID`/`FLEET_ADMISSION_STARTED_AT`；Fleet wrapper 逐字复制它们，原子写入且只写一次 validation-identity.json（required snapshot 必须来自冻结 PRD，不得取 proposer capability snapshot）；再据本脚本真实 exit code 写 fleet-run.json；Evaluator 再写 evaluator.json；Independent Judge 读取并引用 Evaluator 后写 independent-judge.json；控制面最后写 merge-gate.json。所有文件逐字引用 identity SSOT 的 logical_run_id/validation_attempt_id/final_sha，且最终 Vitest 仍从控制面环境读取同一外部锚点对账，证据包自身不能覆盖。五份证据齐备后，从 proposer 合同分支运行 npx vitest run sprints/08032258-kernel-pr1581-fleet-validation-r12/tests/fleet-validation-evidence.test.ts --reporter=verbose，不得在精确 SHA 产品 worktree 内寻找本轮合同测试。

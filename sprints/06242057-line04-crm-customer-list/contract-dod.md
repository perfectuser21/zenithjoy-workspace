...
- [ ] [BEHAVIOR:E2E:COOKIE-SEAM] **cookie 接缝真目标验证**（linux CI 真后端 + 真 better-auth cookie 注入浏览器，无 page.route stub / 无 VITE_SKIP_AUTH）
  ...
  说明（leg 封装）：真浏览器 playwright 调用封装在 line04-crm-customer-list-smoke.sh 的 --leg=cookie-seam 分支内（该 smoke 起 dashboard preview + npx playwright install chromium + 真登录 bootstrap 导出 REAL_SESSION_COOKIE 再跑 crm-cookie-seam.spec.ts，无 stub；cookie 未 bootstrap 出来时 spec 内 test.skip 降级 logic-done-pending）。本 BEHAVIOR **不内联** playwright 命令（避免空默认 env 前缀触发 gate），只调脚本 + psql 复核真写入。
  Test: manual:bash -c 'set -e; bash .github/workflows/scripts/smoke/line04-crm-customer-list-smoke.sh --leg=cookie-seam; IN=$(PGPASSWORD="$PSQL_PASS" psql -h "$PSQL_HOST" -U "$PSQL_USER" -d "$PSQL_DB" -tAc "SELECT (whitelist @> to_jsonb('"'"'$CONTACT'"'"'::text)) FROM zenithjoy.wechat_cs_account_config WHERE wechat_id='"'"'$CS_WECHAT_ID'"'"' AND updated_at > NOW() - interval '"'"'5 minutes'"'"'"); [ "$IN" = "t" ] || { echo "FAIL: cookie 接缝—点开关后 whitelist 无真写入，cookie 未真到达后端"; exit 1; }; echo OK'
  期望: OK

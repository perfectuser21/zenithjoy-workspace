...
- [ ] [ARTIFACT] sprint E2E 验收脚本落地且由 CI workflow 收集
...
- [ ] [BEHAVIOR] [L2] B-01: partial patch 合并当前配置后越界时返回 400 且零写入
...
- [ ] [BEHAVIOR] [L2] B-02: 非法配置沿用 400 INVALID_CONFIG 与不写库语义
...
- [ ] [BEHAVIOR] [L2] B-03: 合法 update 继续返回 200 并 upsert
...
- [ ] [BEHAVIOR] [L2] B-04: acquisition dispatch route 相关回归全绿
...
- [ ] 共享 Red 命令在旧生产代码上记录 actual 200 / expected 400。
- [ ] 生产实现后 `B-01` 转绿，且共享测试文件零 diff。
- [ ] `B-02`–`B-04` 全绿。
- [ ] exact-head CI、Evaluator 与 Judge 均绑定同一 PR head SHA 并 PASS。
- [ ] PR 保持 OPEN，停在 human-review/blind A/B gate。

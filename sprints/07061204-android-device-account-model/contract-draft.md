...
## Round 2 修订说明（针对上轮 Reviewer 反馈）

- **问题1（scope_match_prd=6）**：Golden Path Step 5 拍板决策"以后上报者为准覆盖，旧设备记录标为失效并写日志告警"，上轮只有前半句被 `resolveDeviceConflict` 覆盖，后半句"标失效+写日志告警"无纯函数/测试覆盖 → 本轮新增 `shouldInvalidateOldDeviceRecord(resolution): Boolean` + `shouldLogConflictAlert(resolution): Boolean` 两个纯函数 + 6 条 JUnit 用例，落地"标失效"与"写告警"两个动作的判定。
- **问题2（risk_registered=5）**：风险登记表补两条（风险3：标失效+写告警落地动作未被自动化验收覆盖的风险；风险4：`shouldSkipScanDueToMutex` 只测布尔透传，三服务共享同一把锁的真实竞态(TOCTOU)未被本 sprint 覆盖）。
- **非阻塞建议**：接缝清单补第 4 条——tenant_id 来源应由服务端按 agent_id 反查，不能信任设备上报值（对齐"同机双租户 deny"那次修复的严格程度），标注为 Generator 阶段实现约束。
...
### `shouldInvalidateOldDeviceRecord(resolution: ConflictResolution): Boolean`
- 输入：`resolveDeviceConflict` 的判定结果
- 输出：仅 `OVERWRITE_EXISTING` 返回 `true`（旧设备记录应被标为失效）；`NO_CONFLICT`/`KEEP_EXISTING_STALE_REPORT` 返回 `false`
- 来源：PRD Golden Path Step 5 后半句（"旧设备记录标为失效"）—— Round 1 遗漏，Round 2 补齐

### `shouldLogConflictAlert(resolution: ConflictResolution): Boolean`
- 输入：`resolveDeviceConflict` 的判定结果
- 输出：仅 `OVERWRITE_EXISTING` 返回 `true`（应写日志告警）；`NO_CONFLICT`/`KEEP_EXISTING_STALE_REPORT` 返回 `false`
- 来源：PRD Golden Path Step 5 后半句（"并写日志告警"）—— Round 1 遗漏，Round 2 补齐
...
**禁用命名**：不得把 `ConflictResolution` 枚举值改名为 `WIN`/`LOSE`/`CONFLICT` 等同义词——PRD 用词是"以后上报者为准覆盖/旧设备记录标为失效"，枚举值固定 `NO_CONFLICT`/`OVERWRITE_EXISTING`/`KEEP_EXISTING_STALE_REPORT`；`ScanReadOutcome`/`AccountOfflineStatus`/`DispatchAccountDecision` 同理固定用测试文件里的枚举名。`shouldInvalidateOldDeviceRecord`/`shouldLogConflictAlert` 必须直接以 `ConflictResolution` 为入参（不得重新发明布尔标志位），确保与 `resolveDeviceConflict` 判定结果 1:1 挂钩。
...
### Step 5: 双端登录冲突覆盖判定（以后上报者为准）+ 旧记录标失效 + 写日志告警
**来源**: `[FROM_PRD]` — Golden Path 具体步骤 5（"若发现某账号新出现在这台设备但已绑定在另一台安卓设备上 → 以后上报者为准覆盖，旧设备记录标为失效并写日志告警"）+ NFR"双端登录冲突"拍板值

**可观测行为**:
- 无既有绑定 → `resolveDeviceConflict` 返回 `NO_CONFLICT`；同设备重复上报 → `NO_CONFLICT`；不同设备且新上报时间戳 ≥ 旧记录时间戳 → `OVERWRITE_EXISTING`；不同设备且新上报时间戳 < 旧记录时间戳（网络延迟导致的过期上报）→ `KEEP_EXISTING_STALE_REPORT`（丢弃过期上报，不覆盖）
- **（Round 2 新增）** 仅当 `resolveDeviceConflict` 判定为 `OVERWRITE_EXISTING` 时，`shouldInvalidateOldDeviceRecord(resolution)` 返回 `true`（落地"旧设备记录标为失效"）且 `shouldLogConflictAlert(resolution)` 返回 `true`（落地"写日志告警"）；`NO_CONFLICT`/`KEEP_EXISTING_STALE_REPORT` 两个函数均返回 `false`（Generator 调用方按此两个布尔判定驱动"标失效"写库动作与"写日志告警"动作，不得自行重新判定）
...
**硬阈值**: 单测全绿，对应 test case `no existing binding (first scan) yields NO_CONFLICT` / `same device rescanning yields NO_CONFLICT` / `different device with later scan timestamp overwrites existing` / `different device with equal scan timestamp overwrites existing (later reporter wins ties)` / `different device with earlier (stale, out-of-order) scan timestamp keeps existing` / `OVERWRITE_EXISTING resolution should invalidate old device record` / `NO_CONFLICT resolution should not invalidate old device record` / `KEEP_EXISTING_STALE_REPORT resolution should not invalidate old device record` / `OVERWRITE_EXISTING resolution should log conflict alert` / `NO_CONFLICT resolution should not log conflict alert` / `KEEP_EXISTING_STALE_REPORT resolution should not log conflict alert`
...
| 4 | tenant_id 来源约束（Round 2 新增，Reviewer 非阻塞建议）：`filterAccountsByTenant`/`ScannedAccount.tenantId` 纯函数只测输入已给定 tenant_id 的过滤逻辑，但真实调用链上 tenant_id **不得信任 Android 设备上报值**，必须由服务端按 agent_id 反查得到（对齐"同机双租户 deny"那次修复的严格程度：设备是不可信输入源，租户边界必须在服务端裁定）| 依赖 Generator 在调用方（写入 `agent_platform_sessions` 前）的真实实现是否遵守此约束，纯函数层面无法验证调用方传参来源 | **Generator 阶段实现约束**：Generator 实现"扫描结果写回 `agent_platform_sessions`"的调用代码时，`tenantId` 参数必须来自服务端 `agent_id → tenant_id` 反查（如已有 agent-tenant-resolver），不得直接使用 Android agent 请求体里携带的 tenant_id 字段；本 sprint 纯函数验收无法覆盖这条，需 code review 阶段人工核实调用方传参来源 |
...
| 3（Round 2 新增）| Golden Path Step 5 后半句"标失效+写日志告警"的落地动作（调用方在拿到 `OVERWRITE_EXISTING` 后是否真的执行了 DB 标失效写操作 + 日志告警写操作）本身超出纯函数验收边界——`shouldInvalidateOldDeviceRecord`/`shouldLogConflictAlert` 只保证"该不该做"的判定正确，不保证 Generator 调用方"真的做了" | 若 Generator 调用了这两个纯函数但拿到 `true` 后未真正执行标失效/写日志的副作用代码，问题不会被本 sprint 自动化验收发现，旧设备记录可能悄悄残留为有效状态 | Mitigation 指向本轮问题1修复：已新增 `shouldInvalidateOldDeviceRecord`/`shouldLogConflictAlert` 两个纯函数把"该不该做"显式判定出来并测试覆盖，缩小了 Round 1 完全无覆盖的缺口；"真的做了"这一层调用方副作用验证超出本 sprint `target_environment=local_api` 纯函数验收范围，需 code review + 未来 sprint 补充调用方集成测试 | 部分缓解（判定逻辑已覆盖），调用方副作用执行仍待人工 code review |
| 4（Round 2 新增）| `shouldSkipScanDueToMutex` 只测试了"给定布尔值 → 透传/取反"的纯函数逻辑，三个无障碍服务（切换账号扫描 + 采集 + 触达）共享同一把全局互斥锁时的真实竞态条件（TOCTOU：判定时锁未被占用，但判定后、实际开始扫描前的窗口期锁被其他服务抢占）未被本 sprint 覆盖 | 若真实实现在"检查锁状态"和"开始扫描"之间存在时间窗口且无原子性保证，可能出现两个服务同时操作微信 UI 导致互相干扰（对应 PRD 边界情况段提到的"扫描和触达共用微信窗口"风险类型）| 标注为超出本 sprint 范围：本 sprint `target_environment=local_api` 只做纯函数级 mutex 判定逻辑验收；真实互斥锁的原子获取/释放/TOCTOU 竞态是 Generator 集成代码的职责，列入接缝清单（需未来 sprint 或人工真机验证互斥锁在三服务并发下的原子性）| 未缓解，登记为已知范围外风险，列入接缝清单供未来 sprint 跟进 |
...
if [ -z "${ANDROID_HOME:-}" ]; then
  ...
  [ "$TEST_COUNT" -ge 26 ] || { echo "FAIL: 测试用例数 $TEST_COUNT < 26，疑似删测试（Round 2 新增 6 条后总数应为 29）"; exit 1; }

  echo "✅ Part A 通过（去重/冲突覆盖/标失效+告警判定/tenant绑定/数据保鲜/下线判定/重扫触发/互斥锁，共 $TEST_COUNT 条用例全绿）"
fi
...
| 冲突覆盖后"标失效+写日志告警"判定纯函数（Round 2 新增，补齐 Step 5 后半句覆盖）| `tests/DeviceAccountModelLogicTest.kt` | `OVERWRITE_EXISTING resolution should invalidate old device record`, `NO_CONFLICT resolution should not invalidate old device record`, `KEEP_EXISTING_STALE_REPORT resolution should not invalidate old device record`, `OVERWRITE_EXISTING resolution should log conflict alert`, `NO_CONFLICT resolution should not log conflict alert`, `KEEP_EXISTING_STALE_REPORT resolution should not log conflict alert` | → 编译失败（`shouldInvalidateOldDeviceRecord`/`shouldLogConflictAlert` 未定义）6 failures |

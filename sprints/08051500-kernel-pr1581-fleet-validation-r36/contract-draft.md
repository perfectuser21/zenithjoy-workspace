...
- R7-1 closure：新增可执行依赖预检，真实执行 GitHub API 与当前 attempt Postgres `SELECT 1`；任何失败明确输出 `ENVIRONMENT_FAILURE:<dependency>` 并以 exit 75 结束，绝不进入业务 PASS。
- R7-2 closure：负向 oracle 改为严格要求 `status=failed`、`failure_class=validation_input_invalid`，并要求 `error_message` 点名对应字段；删除“任意非空 metadata 即算失败证据”的假满足面。
- R7-3 closure：冻结测试拆成与 DoD 一致的六个命名用例：正确 payload、错误 repo、缺失/畸形/不一致 SHA、缺失/不可解析 anchor、依赖预检；Test Contract 与 `-t` 覆盖名逐字匹配。
...
**硬阈值**: 七种字段变异全部 `failed/validation_input_invalid` 且错误点名字段；依赖预检失败必须 exit 75 并输出 `ENVIRONMENT_FAILURE`，不得标业务通过。
...
negative mismatched_head target_head_sha "$(make_payload perfectuser21/zenithjoy-workspace 0000000000000000000000000000000000000000 line02/keyword_acquisition#step7)"
...
|---|---|---|---|
| Fleet 真链路 | `sprints/08051500-kernel-pr1581-fleet-validation-r36/tests/fleet-worker-production-chain.test.ts` | 正确 payload 经真实 Fleet Worker 绑定目标；错误仓库 fail-closed；缺失 target_head_sha fail-closed；畸形或不一致 SHA fail-closed；缺失或不可解析锚点 fail-closed；GitHub 与 Postgres 依赖预检 | 当前生产链未满足任一断言即测试 Red |

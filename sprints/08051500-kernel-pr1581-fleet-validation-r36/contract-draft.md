...
- R4-1 closure：将 Step 3 的新鲜 DB 行 oracle 从“恰为 1”统一为“至少 1”；同一 attempt 内 B-01/E2E 为接缝稳定性明确双跑，因而可能产生至少 2 行，而 Step 3 只需证明当前 Worker 执行确有新鲜落账。该改动消除了正确双跑与单行上限之间的矛盾，同时保留五分钟时间窗防历史数据假绿。
...
psql "$DB_URL" -tAc "SELECT count(*) FROM harness_validation_receipts WHERE attempt_id='${HARNESS_ATTEMPT_ID}' AND target_head_sha='c305f6217da65bb69413c39e621b7e797e0fb189' AND created_at>NOW()-interval '5 minutes'" | grep -Eq '^[1-9][0-9]*$'
```
**硬阈值**: provenance 与当前 Runner 精确相等；新鲜 DB 行至少 1（同一 attempt 的接缝双跑允许并预期产生多行）。

...
- 完整性：顶层 keys 必须恰为 `["evidence","failure_class","target","verdict"]`。
- 禁用字段名：`repo`、`head_sha`、`anchor`、`workspace_head`（禁止同义替代或回退工作区 HEAD）。

错误结果保持同一顶层 schema，`verdict="FAIL"`，并给出非空 `failure_class`；不得产生成功结论。
...
**验证命令**:
```bash
node scripts/harness/verify-fleet-target.mjs --payload "$HARNESS_PAYLOAD_PATH" --result "$HARNESS_RESULT_PATH" --product-map product-map/generated/product-map.json --pr 1581
jq -e 'keys==["evidence","failure_class","target","verdict"] and .verdict=="PASS" and .failure_class==null and .target.base_repo=="perfectuser21/zenithjoy-workspace" and .target.target_head_sha=="c305f6217da65bb69413c39e621b7e797e0fb189" and .target.gp_anchor=="line02/keyword_acquisition#step7" and .target.base_sha=="676fed7de12023d355deac7849af8a525ae53f8d" and .evidence.github_pr_head_sha==.target.target_head_sha and .evidence.checked_commit_sha==.target.target_head_sha' "$HARNESS_RESULT_PATH"
```
...
**可观测行为**: 分别删除/篡改三个权威字段，或制造依赖失败，验证器均非零退出且不得输出 PASS。

**验证命令**:
```bash
bash -c 'set -euo pipefail; for expr in "del(.base_repo)" ".target_head_sha=\"short\"" ".gp_anchor=\"line02/keyword_acquisition#step6\""; do P=$(mktemp); jq "$expr" "$HARNESS_PAYLOAD_PATH" > "$P"; if node scripts/harness/verify-fleet-target.mjs --payload "$P" --result "$HARNESS_RESULT_PATH" --product-map product-map/generated/product-map.json --pr 1581 >/tmp/fleet-negative-$$.log 2>&1; then rm -f "$P" /tmp/fleet-negative-$$.log; exit 1; fi; ! grep -q "verdict.*PASS" /tmp/fleet-negative-$$.log; rm -f "$P" /tmp/fleet-negative-$$.log; done'
```
**硬阈值**: 每个变体非零退出；输出不含 PASS；总命令 exit 0。
...
node scripts/harness/verify-fleet-target.mjs --payload "$HARNESS_PAYLOAD_PATH" --result "$HARNESS_RESULT_PATH" --product-map product-map/generated/product-map.json --pr 1581 | tee "$SESSION_TMP/verifier.log"
jq -e 'keys==["evidence","failure_class","target","verdict"] and .verdict=="PASS" and .failure_class==null and .target=={"base_repo":"perfectuser21/zenithjoy-workspace","target_head_sha":"c305f6217da65bb69413c39e621b7e797e0fb189","gp_anchor":"line02/keyword_acquisition#step7","base_sha":"676fed7de12023d355deac7849af8a525ae53f8d"} and .evidence.github_pr_head_sha==.target.target_head_sha and .evidence.checked_commit_sha==.target.target_head_sha' "$HARNESS_RESULT_PATH"
for expr in 'del(.base_repo)' '.base_repo="wrong/repo"' 'del(.target_head_sha)' '.target_head_sha="short"' 'del(.gp_anchor)' '.gp_anchor="line02/keyword_acquisition#step6"'; do
  jq "$expr" "$HARNESS_PAYLOAD_PATH" > "$SESSION_TMP/bad.json"
  if node scripts/harness/verify-fleet-target.mjs --payload "$SESSION_TMP/bad.json" --result "$HARNESS_RESULT_PATH" --product-map product-map/generated/product-map.json --pr 1581 > "$SESSION_TMP/negative.log" 2>&1; then exit 1; fi
  ! grep -q '"verdict":"PASS"' "$SESSION_TMP/negative.log"
done
...
- 中途中断: GitHub 请求中断或 result JSON 半写入，必须 environment_failure，不得 PASS。
...
|---|---|---|---|
| Fleet payload 与证据绑定 | `sprints/08051500-kernel-pr1581-fleet-validation-r36/tests/fleet-target-validation.test.ts` | 接受精确冻结目标并拒绝字段漂移；拒绝 result 证据 SHA 漂移；拒绝 GP 锚点不存在 | verifier 尚未实现，import/执行失败 |

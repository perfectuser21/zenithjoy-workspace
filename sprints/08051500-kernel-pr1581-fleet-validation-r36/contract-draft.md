...
- R6-1 closure：删除 Round 6 的独立 `verify-fleet-payload.mjs` 及其自定义 CLI/JSON schema；本轮所有行为断言都经真实 Brain task 入口派发 `harness_initiative`，再读取真实 Fleet Worker 产生的 task/result/evidence。
- R6-2 closure：负向矩阵补齐错误仓库、缺失/畸形/与 PR head 不一致 SHA、缺失/不可解析锚点，且每项都要求真实任务终态不是 `completed`。
- R6-3 closure：把“测试驱动器误当生产 Worker”的验收剧场风险登记进接缝清单、判定点与未覆盖链路；禁止测试自行生成成功结论或 receipt。
...
- GitHub/Postgres 真宕机不可由合同主动制造；以生产依赖探针不可用时真实任务的 `environment_failure` 为 oracle。若本轮依赖始终健康，该两条仅为 `logic-done-pending`，不得宣称故障接缝已真验。
...
**可观测行为**: 错 repo、缺失/畸形/错 head、缺失/不可解析 anchor 均不能 completed；依赖不可用不能业务 PASS。

**验证命令**: `bash -c 'for ID in ${FLEET_NEGATIVE_TASK_IDS:?}; do curl -sf localhost:5221/api/brain/tasks/$ID | jq -e '\''.status!="completed" and ((.error_message//.failure_class//.metadata//{})|tostring|length>0)'\''; done'`

**硬阈值**: 七种字段变异全部非 completed 且有失败证据；环境故障不得标业务通过。
...
gh api repos/perfectuser21/zenithjoy-workspace/pulls/1581 | jq -e '.head.repo.full_name=="perfectuser21/zenithjoy-workspace" and .head.sha=="c305f6217da65bb69413c39e621b7e797e0fb189" and .base.sha=="676fed7de12023d355deac7849af8a525ae53f8d"'
...
negative() { local name="$1" json="$2"; local id; id=$(submit "$json"); wait_terminal "$id" "$D/$name.json"; jq -e '.status!="completed" and ((.error_message//.failure_class//.metadata//{})|tostring|length>0)' "$D/$name.json"; }
negative wrong_repo "$(make_payload wrong/repo c305f6217da65bb69413c39e621b7e797e0fb189 line02/keyword_acquisition#step7)"
negative malformed_head "$(make_payload perfectuser21/zenithjoy-workspace HEAD line02/keyword_acquisition#step7)"
negative mismatched_head "$(make_payload perfectuser21/zenithjoy-workspace 0000000000000000000000000000000000000000 line02/keyword_acquisition#step7)"
negative bad_anchor "$(make_payload perfectuser21/zenithjoy-workspace c305f6217da65bb69413c39e621b7e797e0fb189 line02/keyword_acquisition#step999)"
for field in base_repo target_head_sha gp_anchor; do negative missing_$field "$(echo "$GOOD" | jq "del(.payload.$field)")"; done
...
|---|---|---|---|
| Fleet 真链路 | `sprints/08051500-kernel-pr1581-fleet-validation-r36/tests/fleet-worker-production-chain.test.ts` | 正确 payload 经真实 Fleet Worker 绑定目标；错误仓库 fail-closed；畸形或不一致 SHA fail-closed；不可解析锚点 fail-closed | 当前生产链未满足任一断言即测试 Red |

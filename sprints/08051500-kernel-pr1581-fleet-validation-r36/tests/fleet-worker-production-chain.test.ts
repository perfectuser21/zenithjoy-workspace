...
describe('Fleet Worker production chain [BEHAVIOR]', () => {
  ...
  it('真实 Fleet task 的依赖失败分类', async () => {
    const endpoint = process.env.FLEET_DEPENDENCY_FAILURE_ENDPOINT;
    expect(endpoint, 'Fleet 必须注入 attempt 隔离故障演练端点；测试自行 POST 正确 payload 并取得本次 task id').toBeTruthy();
    const failureTaskId = curlJson(['-X', 'POST', endpoint!, '-H', 'content-type: application/json', '-d', JSON.stringify(payload())]).id as string;
    expect(failureTaskId).toBeTruthy();
    await terminal(failureTaskId);

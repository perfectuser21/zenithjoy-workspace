...
- [ ] [BEHAVIOR] [L2] B-07: 隔离故障演练的真实 Fleet task 必须环境失败 [接缝×2]
  动作: 向 Fleet 注入的 FLEET_DEPENDENCY_FAILURE_ENDPOINT POST 正确三字段 payload；端点在本 attempt 隔离资源内断开 GitHub 或 Postgres 并返回新 task id
  预期观察: 该任务只能为 failed/environment_failure，错误点名 github 或 postgres；completed 明确 FAIL
  等待预算: 7200s
  留证: POST 响应 task id、隔离故障操作记录与 Brain task 失败 JSON（均含当前 session id）

...
---
# Contract DoD — Sprint: Agent 模块化架构 E2E 验证（Round 2）
...
- [ ] [BEHAVIOR] ok:true 时 fixGuide 字段不出现，且 skipped 不出现（禁用字段反向检查 — keys 完整性）
...
- [ ] [BEHAVIOR] syncModules 收到 active module → downloadImpl 被调用恰好 1 次（PRD 场景A Step 2 覆盖）
...
- [ ] [BEHAVIOR] activateModule → forkImpl 被调用 + config 消息发送 + getActiveModules() 含 lineId（PRD 场景A Step 4 覆盖）
...
- [ ] [BEHAVIOR] GET /api/agent/module-health 响应 data[0].module_status["line04-wechat-cs"].ok == true，子字段 oracle（PRD 场景A Step 5 覆盖；需 API 可用）
...
- [ ] [BEHAVIOR] CI workflow 真正调用 smoke 脚本 + npx tsx（ci_workflow_alignment — Step 7 覆盖）

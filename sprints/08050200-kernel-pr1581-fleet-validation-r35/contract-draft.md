...
| 编号 | closure |
|---|---|
| R5-1 | 删除 ARTIFACT-02 与最终 E2E 对 authoring workspace SHA `49fa4e...` 的字面硬编码；范围检查仅 late-bind 到实际 Generator Runner 注入的 `HARNESS_WORKSPACE_START_SHA`，并用 `git rev-parse --verify` 确认它是 commit。这样未来 Generator 基于其真实冻结起点执行时不会被旧 Proposer 基线误拒，同时仍能机械限制改动只落在本 sprint 目录。 |
...
git rev-parse --verify "${HARNESS_WORKSPACE_START_SHA}^{commit}" >/dev/null

...
- 输入或业务验收失败：`base_repo_missing|base_repo_mismatch|target_head_sha_missing|target_head_sha_invalid|target_head_sha_mismatch|gp_anchor_missing|gp_anchor_invalid|production_oracle_failed`。
...
check_github_head() {
  local payload_file="$1" expected actual
  expected=$(jq -er '.target_head_sha | select(type=="string" and test("^[0-9a-f]{40}$"))' "$payload_file") || { fail_input target_head_sha_invalid; return 20; }
  HEAD_SHA="$expected"
...
# GitHub 真调用成功后与结构化 payload 的另一完整 SHA 比较；目标只能从权威 payload 读取。
jq '.target_head_sha="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"' "$EVIDENCE_DIR/payload.json" >"$EVIDENCE_DIR/github-mismatch-payload.json"
if GH_MISMATCH_RECEIPT=$(check_github_head "$EVIDENCE_DIR/github-mismatch-payload.json"); then

...
const expected = {
  ...
  attemptId: 'c2f3006f-3c99-44db-8ad7-34db60580c9e',
  ...
  snapshotId: '12e25b55-9b11-452b-95dc-a82e03984211',
...
function expectFresh(name: string, manifest: Json, artifact: Json): void {
  const attemptStarted = Date.parse(manifest.attempt_started_at);
...
function expectCommonBinding(artifact: Json): void {
  expect(artifact.run_id).toBe(expected.runId);
  expect(artifact.attempt_id).toBe(expected.attemptId);
...
describe('PR #1581 real fleet validation evidence [BEHAVIOR]', () => {
  it('精确 PR HEAD 与冻结基线及获准 US M4 能力绑定', () => {
    ...
    expect(manifest).toMatchObject({
      ...
      model: 'gpt-5.6-sol',
      machine: 'us-mac-m4',
      capability_snapshot_id: expected.snapshotId,

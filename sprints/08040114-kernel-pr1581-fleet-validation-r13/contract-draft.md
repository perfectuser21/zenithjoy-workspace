...
**可观测行为**: 仅当两份裁决通过、新鲜且 SHA 一致时 `merge_allowed=true`；任何缺证、旧证、漂移或能力不符时 reasons 非空并保持不可合并。

**验证命令**:
```bash
npx vitest run sprints/08040114-kernel-pr1581-fleet-validation-r13/tests/fleet-validation-evidence.test.ts -t "机械合并门仅在双裁决新鲜且 SHA 一致时放行" --reporter=verbose
```
**硬阈值**: 1/1 exit 0；`merge_allowed=true`、reasons=`[]`、Evaluator/Judge/gate 四处 final SHA 完全一致；本角色与 Generator 均不得执行 merge。
...
```
**硬阈值**: 正式证据 4/4 exit 0；缺任一必需文件或任一字段漂移时至少 1 test fail，禁止 404/skip/`|| true` 旁路。

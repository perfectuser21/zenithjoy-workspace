# -*- coding: utf-8 -*-
"""合同测试骨架 — Line04 半死区修复（task 5e9d608f）
...
本文件是合同测试骨架（commit-1 TDD），实现代码（listen_chat.py 修改）在 commit-2 落地。
"""
...
# ---------------------------------------------------------------------------
# 路径与 stub：不依赖 pywinauto、requests、ctypes（macOS/Linux CI 可跑）
# ---------------------------------------------------------------------------
...
def _make_ct(*, zoomed: int, iconic: int, w: int = 1920) -> types.SimpleNamespace:
    ...
    GetWindowRect 通过修改传入的 RECT 引用返回宽度，但 assert_window_shape_for_header
    的实现可能只用 IsZoomed/IsIconic；w 参数留作未来宽度分支测试用。
    """
    import ctypes

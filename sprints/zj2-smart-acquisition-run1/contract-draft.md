...
### Step 3: 中台返回 task_id 后写入 acquisition_keyword_tasks 并向 Agent 下发搜索任务

**来源**: `[AI_ADDED]` — GAN Round 1 Proposer 加入；理由：PRD 只说"派搜索任务给主号 Agent"但未定义任务追踪机制，不写 DB 则 Agent 离线后无法重试/追踪，evaluation 无法验证任务是否真的下发
...
[ "$STATUS" = "dispatched" ] || [ "$STATUS" = "pending" ] || \
  { echo "FAIL: status 不是 dispatched/pending，实际=$STATUS"; exit 1; }
echo "✅ Step 3 任务状态验证通过"
```

**硬阈值**: `status` 为 `dispatched` 或 `pending`（Agent 已接收或等待中）
...
- `leads` 为数组，每条含 `commenter_id`, `comment_text`, `source_video_url`, `crawled_at`, `grade`, `keyword`
...
# grade 枚举合法性（若有记录）
GRADE_OK=$(echo "$RESP" | jq -e '.leads | map(.grade) | all(. == "感兴趣" or . == "精准" or . == "高意向")' 2>/dev/null && echo "1" || echo "0")
...
echo "$FILTERED" | jq -e '.leads | map(.grade) | all(. == "高意向")' || { echo "FAIL: grade 筛选不正确"; exit 1; }

# grade 非法值 → 400
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:3001/api/acquisition/leads?grade=invalid")
[ "$STATUS" = "400" ] || { echo "FAIL: 非法 grade 未返回 400，实际=$STATUS"; exit 1; }

echo "✅ Step 7 GET /api/acquisition/leads 验证通过"
```

**硬阈值**: HTTP 200，keys = `["leads","total"]`，grade 合法，grade=invalid → 400，耗时 < 5s
...
# Mode A API assertions in WS4 DoD BEHAVIOR 条目
# Mode B Playwright browser E2E in ## E2E 验收 区块
npx playwright test --project=chromium e2e/acquisition-leads.spec.ts
```
...
# 2. POST keyword-search → 验证 schema
$body = '{"keyword":"装修"}'
...
# 5. GET /api/acquisition/leads → schema 验证
$leads = Invoke-RestMethod -Uri "$ApiBase/api/acquisition/leads" -Method GET -TimeoutSec 10
...
# 6. grade 筛选
$filtered = Invoke-RestMethod -Uri "$ApiBase/api/acquisition/leads?grade=高意向" -Method GET -TimeoutSec 10
...
# 7. grade 非法值 → 400
try {
  Invoke-RestMethod -Uri "$ApiBase/api/acquisition/leads?grade=invalid" -Method GET -TimeoutSec 10
...
# 8. Playwright — /dashboard/leads 页面 UI 验证
$playwrightScript = @"
...
| WS2 | `tests/ws2/keyword-search.test.ts` | POST keyword-search schema/DB | endpoint 不存在 → 404 |
| WS3 | `tests/ws3/video-comment-result.test.ts` | video-result/comment-result schema | endpoint 不存在 → 404 |
| WS4 | `tests/ws4/leads.test.ts` | GET leads schema / grade 筛选 / 404 | endpoint 不存在 → 404 |

---

## PRD Response Schema → jq -e 自查（v7.5 checklist）

1. **PRD 字段名**: `task_id`, `keywords`（keyword-search）；`leads`, `total`（leads API）
2. **Contract jq -e 字段名**: `task_id`, `keywords`, `leads`, `total` — 与 PRD 完全字面匹配 ✅
3. **禁用字段反向检查**: `result`, `data`, `expanded`, `variants`, `id`, `job_id`, `items`, `records`, `rows` — 均以 `has("X") | not` 形式写入，无正向断言 ✅
4. **BEHAVIOR 数量**: 每个 ws DoD ≥ 4 条 [BEHAVIOR]（含 schema 字段 + keys 完整性 + 禁用字段反向 + error path）✅

---

## GAN 对抗焦点

- Step 1 验证命令加 `-sf` flag（HTTP 5xx 返回非 0），且加 `| jq -e 'keys == ...'` 精确匹配防 schema drift
- Step 2 DB 查询加 `AND created_at > NOW() - interval '5 minutes'` 防历史数据绕过
- Step 4/5 acquisition_videos 查询同样加时间窗口

...
DATA_DIR="$HOME/.$PLATFORM"

case "$ACTION" in
  start)
    if docker ps | grep -q "$CONTAINER"; then
      echo "{\"status\":\"already_running\",\"platform\":\"$NAME\",\"vnc_url\":\"http://$VPS_IP:$PORT/vnc.html\"}"
      exit 0
    fi
    ...
    echo "{\"status\":\"ready\",\"platform\":\"$NAME\",\"vnc_url\":\"http://$VPS_IP:$PORT/vnc.html\",\"message\":\"请在 VNC 中扫码登录$NAME\"}"
    ;;

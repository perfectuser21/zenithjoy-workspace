...
#   - scripts/nas-fetch-content.sh（仓库根 scripts/ 下）
#   - 同目录下 publish-kuaishou.cjs、publish-weibo.cjs、publish-micro.cjs
# ============================================================
...
# scripts/ → publisher/ → workflows/ → platform-data/ → workflows/ → 仓库根
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
PUBLISHER_DIR="$SCRIPT_DIR"
...
if [[ -z "$CONTENT_ID" ]]; then
    ...
    echo "用法: bash publish-by-content-id.sh --content-id <id> --platform <kuaishou|weibo|toutiao> [--account <账号>] [--dry-run]"
    exit 1
fi
...
ACCOUNT_ARG="${ACCOUNT:-}"
python3 - "$PLATFORM" "$CONTENT_ID" "$BODY_TEXT" "$TITLE" "$IMAGES_JSON" "$ACCOUNT_ARG" <<'PYEOF' > "$PUBLISH_JSON"
import json, sys
data = {
    "type":    sys.argv[1],
    "id":      sys.argv[2],
...
case "$PLATFORM" in
    kuaishou)
        if node "$PUBLISHER_DIR/publish-kuaishou.cjs" --content "$PUBLISH_JSON" 2>&1 | tee "$PUBLISHER_OUT"; then
            ...
            PLATFORM_POST_ID=$(grep -o 'PLATFORM_POST_ID:.*' "$PUBLISHER_OUT" 2>/dev/null | head -1 | cut -d: -f2 | xargs || true)
        else
...
_write_fail_log() {
    local err_json
    err_json=$(python3 -c "import json,sys; print(json.dumps(sys.argv[1]))" "$1" 2>/dev/null || echo "NULL")
    PGPASSWORD="${POSTGRES_PASSWORD:-}" psql \
        -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" \
        -c "INSERT INTO zenithjoy.publish_logs (work_id, platform, status, error_message, created_at)
            VALUES ('$WORK_ID', '$PLATFORM', 'failed', $err_json, NOW())" 2>/dev/null \
        || echo "  ⚠️  publish_logs 失败记录写入失败"
}

if [[ "$PUBLISH_SUCCESS" == "false" ]]; then
    echo "❌ 发布失败: $PUBLISH_ERROR"
    _write_fail_log "$PUBLISH_ERROR"
    rm -rf "$TMP_DIR"
    exit 1
fi
...
echo "   work_id:    $WORK_ID"
[[ -n "$ACCOUNT" ]]          && echo "   account:    $ACCOUNT"
[[ -n "$PLATFORM_POST_ID" ]] && echo "   post_id:    $PLATFORM_POST_ID"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

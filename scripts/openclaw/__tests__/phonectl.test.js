...
test('tap 参数映射：x/y 正确进请求体，Authorization 头正确，200 ok:true → exit 0', async () => {
  ...
  const { status, stdout } = runPhonectl([AGENT_ID, 'tap', '540', '1200'], {
...
test('swipe 带 durationMs 时正确解析，不带时用默认（不发 durationMs 字段）', async () => {
  ...
  runPhonectl([AGENT_ID, 'swipe', '10', '20', '30', '40'], {
    ZENITHJOY_API_BASE: `http://127.0.0.1:${server.address().port}`,
...
test('swipe 带显式 durationMs', async () => {
  ...
  runPhonectl([AGENT_ID, 'swipe', '10', '20', '30', '40', '500'], {
    ZENITHJOY_API_BASE: `http://127.0.0.1:${server.address().port}`,
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  await stopServer(server);
...
test('type 文本原样透传（含空格）', async () => {
  ...
  runPhonectl([AGENT_ID, 'type', '人工智能训练师'], {
    ZENITHJOY_API_BASE: `http://127.0.0.1:${server.address().port}`,
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  await stopServer(server);
...
test('key 只认 back|home，其他值本地直接拒绝(不发请求) exit 2', () => {
  const { status, stderr } = runPhonectl([AGENT_ID, 'key', 'menu'], {
...
test('launch pkg 透传', async () => {
  ...
  runPhonectl([AGENT_ID, 'launch', 'com.ss.android.ugc.aweme'], {
    ZENITHJOY_API_BASE: `http://127.0.0.1:${server.address().port}`,
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  await stopServer(server);
...
test('--timeout-ms 与 --idempotency-key 正确进请求体', async () => {
  ...
  runPhonectl([AGENT_ID, 'tap', '1', '2', '--timeout-ms', '20000', '--idempotency-key', ik], {
    ZENITHJOY_API_BASE: `http://127.0.0.1:${server.address().port}`,
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  await stopServer(server);
...
test('data.ok=false → 打印结果但 exit 非零（AI 循环能读到失败原因）', async () => {
  ...
  const { status, stdout } = runPhonectl([AGENT_ID, 'tap', '9999', '9999'], {
    ZENITHJOY_API_BASE: `http://127.0.0.1:${server.address().port}`,
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  await stopServer(server);
...
test('504 超时：outcome:unknown 透传到 stdout，exit 非零', async () => {
  ...
  const { status, stdout } = runPhonectl([AGENT_ID, 'tap', '1', '2'], {
    ZENITHJOY_API_BASE: `http://127.0.0.1:${server.address().port}`,
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  await stopServer(server);
...
test('缺 ZENITHJOY_INTERNAL_TOKEN → 本地直接拒绝(不发请求) exit 2', () => {
  const { status, stderr } = runPhonectl([AGENT_ID, 'tap', '1', '2'], {
    ZENITHJOY_API_BASE: 'http://127.0.0.1:1',
...
test('agent_id 非 uuid 形状 → 本地直接拒绝 exit 2', () => {
  const { status, stderr } = runPhonectl(['not-a-uuid', 'tap', '1', '2'], {
    ZENITHJOY_API_BASE: 'http://127.0.0.1:1',
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  assert.equal(status, 2);
...
test('未知 action → 本地直接拒绝 exit 2', () => {
  const { status, stderr } = runPhonectl([AGENT_ID, 'reboot'], {
    ZENITHJOY_API_BASE: 'http://127.0.0.1:1',
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  assert.equal(status, 2);
...
test('参数不足（tap 缺 y） → 本地直接拒绝 exit 2', () => {
  const { status } = runPhonectl([AGENT_ID, 'tap', '1'], {
    ZENITHJOY_API_BASE: 'http://127.0.0.1:1',
    ZENITHJOY_INTERNAL_TOKEN: 't',
  });
  assert.equal(status, 2);

...
- 配置 `~/.config/zenithjoy/wall.env`（chmod 600）：`ZJ_API_BASE`、`ZJ_LICENSE`、`ZJ_INTERNAL_TOKEN`、可选 `WALL_INTERVAL`（默认 1 秒）、`WALL_WIDTH`（默认 360）、`WALL_CONVERT`（缩图命令，默认 macOS `sips`；测试注入直通）。
...
- 每台手机一个后台子进程推帧：`adb -s S exec-out screencap -p` → `$WALL_CONVERT` 缩到 360 宽 JPEG q55 → 若 >120KB 再以 q35 重压一次，仍超则跳过该帧 → `POST /api/workers/<uuid>/frame`，头 `X-Agent-License: <license>` + `Content-Type: image/jpeg`，`curl -m 5`。返回 429 退避 10 秒；4xx/5xx 只记日志。设备离线时子进程退出，主循环下一轮重拉。
...
- 执行器面三端点**只用内部 token**（`Authorization: Bearer <ZJ_INTERNAL_TOKEN>`），**绝不带 `X-Agent-License`**（带了会被分流到 license 路径而 401）。
  ...
  - `POST /api/workers/tasks/<id>/steps` `{step_index, status, executor_id, note?, foreground_pkg?, diag_line?, screenshot_jpeg_b64?}`；`failed` 必带三件套：前台包名 = `adb shell dumpsys window | grep mCurrentFocus` 正则取包名（取不到填 `unknown`），诊断行 = 参数或最近一条日志，截图 = `adb exec-out screencap -p` 经 `$WALL_CONVERT` 压到 ≤200KB 后 base64（无 data URI 前缀）。
...
| `outreach-tick.sh` | 69 行后 `start --profile "$PROFILE" "触达·单#$SEQ $NICK" "拿锁,发送,核验"`；各分支 `done` / `fail` | 必须在 23 行 source 守卫之后 |
...
| 变异自证 | 帧上限改 1KB → unit 红；删 harvest-cron 挂钩 → smoke 红 |

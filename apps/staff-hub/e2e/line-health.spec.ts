 ...
 *   01-overview.png       总览页 3 张卡片 + 顶部 staging/production 部署摘要
 *   02-detail-deploy.png  点 line04 卡片进详情页，默认"部署"tab 渲染 staging/production 真实版本
 ...
 *
 * 2026-07-29 二次修正：deployment 不再是"dev/staging/production 三环境按 git 分支猜"
 * （dev=develop 分支从未被部署过，staging/production 也曾错误对应到跟真实部署无关的
 * 分支），改为直接查 apps/api 自己的 /version 端点（staging/production 各一次，CI/本地
 * 沙盒够不着这两个内网地址时二者 sha 均为 null，属合法路径，断言只看结构不看具体数据）。
 *
 * 变体C 死规则：所有断言打真实 apps/api（无 stub），因此
 *   - Brain 在 CI/沙盒可能不可达 → 任一线都可能走 degraded 分支，属合法路径（合同「未覆盖真实链路清单」第2条）
 *   - GitHub 未认证可能限流、内网 /version 地址在 CI 沙盒不可达 → sha 可能为 null，断言只看结构不看具体数据
 *
...
test.describe('Staff Hub 业务线健康看板', () => {
  test('总览页渲染 3 张业务线卡片', async ({ page }) => {
    ...
    // 三条线共享的部署摘要（staging/production 真实 /version），CI 沙盒够不着内网地址时
    // sha 显示"不可达"也算合法渲染——只断言这一行真的出现了
    await expect(page.getByTestId('line-deployment-summary')).toBeVisible();

    await page.screenshot({ path: shot('01-overview.png'), fullPage: true });
  });

  test('点击 line04 卡片进详情页，默认部署 tab 渲染 staging/production 真实版本', async ({ page }) => {
    await page.goto('/line-health');

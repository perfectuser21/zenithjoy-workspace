...
// ESM 下 __dirname 派生;goto 用 playwright baseURL 相对路径统一(巡检 2026-07-12 收编修复)
...
test.describe('/module-health 普通账号可访问 [BEHAVIOR]', () => {
  test('普通账号登录后访问 /module-health 不被 403/重定向拒绝', async ({ page }) => {
    ...
    // 不应被踢回首页(pathname='/'),与具体 host/port 无关
    await expect.poll(() => new URL(page.url()).pathname, { timeout: 3000 }).not.toBe('/');
  ...
  test('navigation.config.ts /module-health 无 requireSuperAdmin: true', async () => {
    // 静态文件检测（不依赖浏览器，直接读 TS 文件；fs/path 已在顶部 ESM import）
    ...
    // 只截 /module-health 这一行(每个路由项独占一行),避免固定 250 字符窗口越界到下一行
    // 的 /admin/license 的 requireSuperAdmin 造成误判(巡检 2026-07-12 收编修复)
    const segment = src.slice(mhIndex, src.indexOf('\n', mhIndex));
    expect(
      segment,

...
function formatEventNode(p: EventPayload): { icon: string; label: string; detail: string; accent: 'ok' | 'bad' | 'neutral' } {
  const fileTail = (path?: string) => path ? path.split('/').slice(-2).join('/') : ''
  switch (p.node) {
    case 'research':
      return { icon: '🔬', label: '调研', detail: p.findings_path ? `→ ${fileTail(p.findings_path)}` : '', accent: 'neutral' }
    case 'copywrite':
      return { icon: '✍️', label: '文案生成', detail: p.copy_path ? `→ ${fileTail(p.copy_path)}` : '', accent: 'neutral' }
    case 'copy_review': {
    ...
    case 'generate':
      return { icon: '🎨', label: '图片生成', detail: p.cards_dir ? `→ ${fileTail(p.cards_dir)}/` : '', accent: 'neutral' }
    case 'image_review': {
    ...
    case 'export':
      return { icon: '📦', label: '导出', detail: p.nas_url ? `NAS: ${p.nas_url}` : '', accent: 'ok' }
    default:

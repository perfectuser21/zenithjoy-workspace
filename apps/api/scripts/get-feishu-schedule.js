#!/usr/bin/env node
/**
 * get-feishu-schedule.js — 输出"内容排期"表最新一条 / 字段值
 *
 * Usage:
 *   node apps/api/scripts/get-feishu-schedule.js --customer=<c> --latest [--field=<f>]
 *   --help    Show this help
 */
const helper = require('./_feishu-helper');

const USAGE = `Usage: node get-feishu-schedule.js --customer=<c> --latest [--field=<f>]
  --customer  关联客户（必填）
  --latest    取最新一条（必填，标志位）
  --field     仅输出指定字段值（可选）
  --help      Show this help`;
...
async function main() {
  ...
  const r = await helper.postJson(
    url,
    {
      // 内容排期表的客户字段假定通过其他形式（如文案前缀或外键）关联，thin 阶段先按文案模糊
      filter: { conjunction: 'and', conditions: [{ field_name: '文案', operator: 'contains', value: [String(args.customer)] }] },

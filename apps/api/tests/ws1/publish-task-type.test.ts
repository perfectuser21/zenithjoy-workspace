...
describe('Workstream 1 — publish_tasks.type 字段 + createPublishTask 接 type [BEHAVIOR]', () => {
  ...
  it('createPublishTask({type:"video"}) 持久化 type=video 到 DB', async () => {
    const { createPublishTask } = await import('../../src/services/walking-skeleton.service');
    const task = await (createPublishTask as any)({
    ...
    const dbType = psql(`SELECT type FROM zenithjoy.publish_tasks WHERE id='${task.id}';`);
    expect(dbType).toBe('video');
  });

  it('POST /api/publish/task 缺 type 字段 → 422 或缺省 image (策略需明确)', async () => {
    const app = (await import('../../src/app')).default;
    const res = await request(app).post('/api/publish/task').send({ platform: 'douyin', payload: {} });
  ...
  it('POST /api/publish/task type="banana" → 422 invalid value', async () => {
    const app = (await import('../../src/app')).default;
    const res = await request(app).post('/api/publish/task').send({ platform: 'douyin', type: 'banana', payload: {} });
    expect([400, 422]).toContain(res.status);

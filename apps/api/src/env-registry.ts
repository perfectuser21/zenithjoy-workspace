...
export const OPTIONAL_ENV: { name: string; reason: string }[] = [
  ...
  { name: 'GRADING_BASE_URL', reason: '评论意向判定链的网关地址，缺则回退 TOAPIS_BASE_URL、再缺用默认 https://openrouter.ai/api/v1（0922 从 ToAPIs 迁来：那边多渠道 403 欠费/Azure 内容过滤/批量超时，无一模型能同时过三关）' },
  { name: 'GRADING_API_KEY', reason: '评论意向判定链的凭据，缺则依次回退 OPENROUTER_API_KEY、TOAPIS_API_KEY；全缺则跳过判定、整批返 null（静默分支，症状与渠道欠费相同）' },
  ...
  { name: 'OPENROUTER_API_KEY', reason: 'OpenRouter 凭据，缺则不走 OpenRouter 路径；0922 起也是评论意向判定链的默认网关凭据（GRADING_API_KEY 未设时用它）' },
  ...
  { name: 'VOLCENGINE_TTS_APP_ID', reason: '批量混剪配音刀(GP line05/batch_mashup#step4)火山引擎 TTS 凭据，缺则 tts-volcengine.ts 的 synthesize() 抛 VolcengineTtsError(missing_credentials)，不阻断中台启动，由调用方（渲染层）决定兜底' },
  { name: 'VOLCENGINE_TTS_ACCESS_KEY', reason: '同 VOLCENGINE_TTS_APP_ID——火山引擎 TTS access_key，凭据均在 1Password CS「Volcengine Speech (豆包实时语音对话)」条目' },
  { name: 'VOLCENGINE_TTS_BASE_URL', reason: '火山 TTS 接口地址，缺则用默认 https://openspeech.bytedance.com/api/v1/tts（实测通路）' },
  { name: 'VOLCENGINE_TTS_RETRY_BASE_MS', reason: '火山 TTS 网关类错误(520/502/504等)重试退避基数毫秒，缺则默认 1000；单测调成 1 免得白等，生产一般不设。口径同 TOAPIS_RETRY_BASE_MS（issue f3b6ba7c）' },

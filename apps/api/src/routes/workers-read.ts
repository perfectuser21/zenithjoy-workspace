...
workersReadRouter.get('/:agentId/activity', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const shotUrl = (ref: unknown) => (typeof ref === 'string' && ref ? `/api/workers/shots/${ref}` : null);
    const withUrl = (s: Record<string, unknown>) => ({ ...s, screenshot_url: shotUrl(s.screenshot_ref) });
    const historyWithUrl = (h: (typeof a.history)[number]) => ({
      ...h,
      evidence_screenshot_url: shotUrl(h.evidence_screenshot_ref),
      failed_scene: h.failed_scene ? { ...h.failed_scene, screenshot_url: shotUrl(h.failed_scene.screenshot_ref) } : null,
    });
    ...
    return res.json(OK({ current: a.current, steps: a.steps.map(withUrl), history: a.history.map(historyWithUrl), frame_age_ms }));

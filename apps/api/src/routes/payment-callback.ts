...
// CodeQL js/missing-rate-limiting：这是整条链路唯一公网可达且无身份鉴权的入口，
// 每次请求都会做一次 RSA 验签（CPU 开销），不挡在最前面等于让匿名扫描白嫖验签成本。
//
// 按 IP 限流（不能按 tenant）：回调发生在 tenantContext 之前，根本没有租户上下文，
// 用默认的 tenantKeyFn 只会把所有平台、所有商户的回调全部落进同一个 'anonymous' 桶，
// 等于变相共用一个全局限流器——一个租户的回调洪峰能把别的租户也顶到 429。
//
// 阈值定宽松而不是定严：微信/支付宝回调源 IP 数量有限但可能密集重推，定太严会把
// 正常回调误判为超限 → 我方返 429 → 支付平台判定投递失败 → 继续重推或最终放弃 →
// 订单永久入不了账，丢钱比被扫更贵。300 次/分钟/IP 远高于任何正常回调频率，
// 但足够挡住扫描器/DoS 级别的滥用。
// 导出给限流生效测试用，避免测试里另起一个 magic number（阈值改了测试自动跟着改）。
export const CALLBACK_RATE_LIMIT_WINDOW_MS = 60_000;
export const CALLBACK_RATE_LIMIT_MAX = 300;

paymentCallbackRouter.post(
  '/:provider',
  simpleRateLimit({ windowMs: CALLBACK_RATE_LIMIT_WINDOW_MS, max: CALLBACK_RATE_LIMIT_MAX, keyFn: ipKeyFn }),

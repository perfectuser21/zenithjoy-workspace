  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (err instanceof TaskNotFoundError) {
        return res.status(404).json({ ok: false, code: 'TASK_NOT_FOUND', message: (err as Error).message });
      }
      if (err instanceof TaskForbiddenError) {
        return res.status(403).json({ ok: false, code: 'FORBIDDEN', message: (err as Error).message });
      }
      const msg = err instanceof Error ? err.message : 'unknown';

...
describe('HEARTBEAT_MODULES version gates', () => {
  it('line04-wechat-cs required_version should be 1.0.109 (Gap1 agent license 自证上报发货版本，三面一致)', async () => {
    const { HEARTBEAT_MODULES } = await import('./walking-skeleton.service');
    expect(HEARTBEAT_MODULES['line04-wechat-cs'].required_version).toBe('1.0.109');
  });
});

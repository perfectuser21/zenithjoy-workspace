...
目标是去掉 15-25% 的冗余内容，不是大幅删减。有效内容间的气口停顿（句子间呼吸）是正常语言节奏，不要删。

## 保留（keep: true）
- 与主题相关的观点、数据、案例、结论
- 句子之间的气口（即使内容是"嗯""然后"等过渡词，只要两侧有有效内容就保留）
- 举例、逻辑递进、对比分析

## 删除（keep: false）—— 必须满足以下之一
- 完全跑题：与主题毫无关系的闲聊（接电话、和路人说话）
- 严重重复：完全相同的意思已表达过两次以上
- 口误的错误部分：说错后自我纠正，只删错误部分，纠正内容保留
- 孤立无意义：前后均为无效内容，且本身仅有"嗯""啊""哦"独立成段

## 禁止删除
- 不能因句子短就删
- 不能因含停顿词（然后、就是、那个）就删，这是正常口语
- 不能删有效内容之间的气口
...
只输出 JSON，不要其他内容：
{
  "segments": [
    ...
    {"index": 1, "keep": false, "reason": "完全跑题"}
  ]
}

/**
 * 路③ Sprint E 合同测试的环境前置 —— 从连接串推导 apps/api 真正认的那五个变量
 *
 * 与 Sprint B/C/D 的 vitest.workbench-{rows,views,relations}.setup.ts 同口径（连的库必须与判据一致）：
...
if (!raw) {
  throw new Error('[workbench-rollup] 未设 E2E_DATABASE_URL / DATABASE_URL —— 拒绝落缺省库跑成假绿');
}

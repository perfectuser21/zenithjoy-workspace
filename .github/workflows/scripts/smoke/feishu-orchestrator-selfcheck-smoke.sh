#!/usr/bin/env bash
# 飞书编排台同步器启动自检 smoke：CI 环境没有 FEISHU_ORCH_* env，
# 断言 API 进程照常活着（缺配置=红日志跳过启动，绝不 crash 拖垮整个中台）。
# 真正的飞书双向同步是环境接缝（真 Feishu API），CI 测不到——
# 由 vitest（mock 契约 19 用例）+ 合并后 staging 真验兜住。
set -euo pipefail
API_BASE="${API_BASE:-http://localhost:5200}"
fail() { echo "❌ $*"; exit 1; }

echo "[1] 无 FEISHU_ORCH env 下 API 健康"
...
echo "✅ feishu-orchestrator selfcheck smoke PASS"

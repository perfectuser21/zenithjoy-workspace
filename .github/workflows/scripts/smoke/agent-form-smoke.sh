...
#   B) exe 应用图标：package:win 真把 build/icon.ico 写进 exe（pkg 5.x 无 --icon，用 rcedit 后处理）；
...
# ── B) exe 应用图标：package:win 用 rcedit 把 icon.ico 写进 exe ────────────────────────────
echo "[B] exe 应用图标接入（pkg 5.x 无 --icon → rcedit 后处理）"
PKG_WIN=$(node -e "process.stdout.write(require('$PKG_JSON').scripts['package:win']||'')")
echo "  package:win = $PKG_WIN"
echo "$PKG_WIN" | grep -qiE "rcedit|icon\.ico" \
  || { echo "FAIL: package:win 未把 exe 图标（rcedit + icon.ico）接进打包链"; exit 4; }
echo "$PKG_WIN" | grep -qiE "icon\.ico" \
  || { echo "FAIL: package:win 未引用 build/icon.ico"; exit 4; }
# rcedit 必须在 devDependencies（CI windows runner 能装到）
...
echo "  OK: exe 图标接 rcedit + icon.ico"

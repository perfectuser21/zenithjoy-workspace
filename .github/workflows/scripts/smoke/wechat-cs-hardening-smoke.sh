...
# v1.0.17: PostMessageW(WM_KEYDOWN/WM_KEYUP) 直投 main_hwnd 消息队列，xian-pc 2026-06-10 实测有效
# 禁止 send_keys（全局物理注入，后台 session 发到错误窗口）
grep -qE 'PostMessageW.*0x0100|PostMessageW.*WM_KEYDOWN' "$LISTEN" || { echo "FAIL: 未用 PostMessageW Enter（v1.0.17 后台静默必需）"; exit 1; }
! grep -qE "send_keys\('{ENTER}'\)" "$LISTEN" || { echo "FAIL: 仍残留 send_keys Enter（物理全局注入，禁止）"; exit 1; }
...
echo "  PASS UIA SetValue + PostMessageW Enter（后台静默，无物理输入）"

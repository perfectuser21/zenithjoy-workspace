...
# 验证 Line04 CRM Glide 运营台前端依赖的所有后端端点存在且响应正确：
...
#   超管无 cs_wechat_id 属于合规校验（决策5：不一次拉全平台），预期 400。
set -euo pipefail
...
#    拒绝：404（端点未注册）、5xx（崩溃）
STATUS=$(curl -s -o /tmp/crm-glide-customers.json -w "%{http_code}" \
  "$API_BASE/api/crm/customers" \
  -H "X-Internal-Token: $INTERNAL_TOKEN" \
  -H "X-User-Email: $ADMIN_EMAIL")
RESP=$(cat /tmp/crm-glide-customers.json)
if [ "$STATUS" = "200" ]; then
  echo "$RESP" | python3 -c "
import sys, json
...
"
  ...
  # 超管无 cs_wechat_id = 合规校验（决策5：不一次拉全平台）

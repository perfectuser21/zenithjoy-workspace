 ...
 * 多个轮询 loop（彼时含关键词轮询与 collectPollLoop）只在 onDestroy 停止。服务被
 * 多次 start（开机广播 + MainActivity + START_STICKY 重启）后就泄漏出多套并行
 * 轮询 loop，同一个采集任务每个周期被投递 N 次（logcat 实测每 30s 周期两条相隔
 * ~1.7s 的重复投递）。initAgent 必须只执行一次。
 */

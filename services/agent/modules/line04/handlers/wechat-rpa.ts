...
const LISTENER_BACKOFF_STEPS_MS = [30_000, 60_000, 120_000, 300_000];
// 子进程存活超此阈值视为"这次拉起是健康的"，下次失败退避计数器归零（从 30s 重新起）。
const LISTENER_HEALTHY_UPTIME_MS = 10 * 60_000;
...
export function startWechatListener(apiBase: string, agentId?: string, machineId?: string): void {
  ...
  const scheduleListenerRespawn = (reason: string): void => {
    ...
    console.warn(
      `[wechat-rpa] listen_chat.py ${reason}，${delay / 1000}s 后自动重启（崩溃自愈，第 ${_listenerFailCount} 次，退避 ${delay / 1000}s）`,
    );
    setTimeout(() => {
      _listenerKillFuncs.killExistingListeners();
      spawnOnce();

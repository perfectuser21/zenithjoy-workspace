// services/agent/src/handlers/toutiao-publish.ts
//
// 真调 toutiao-publisher 走头条号微头条 dry-run，但 *不上传图片、不点发布按钮*。
//
// 安全约束（v0.3）：
//   只 spawn `publish-toutiao-image-dryrun.cjs`（仅验证已登录 + 拦截发布 API，
//   绝不调 /weitoutiao/post 等真发 API，绝不污染头条公域）。
//   v0.3 仅支持 image (微头条) 模式，文章/视频留待后续。
//
// 输入 payload.content：
...
const SCRIPT_PATH = resolveScriptPath(
  'publishers',
  'toutiao-publisher',
  'publish-toutiao-image-dryrun.cjs',
);
...
function ensureSampleImage(): string {
  const dir = path.join(os.tmpdir(), 'zenithjoy-agent-toutiao');
...
export async function handleToutiaoPublish(
  taskId: string,
  content: ToutiaoContent,
  emit: (msg: any) => void,
  ...
  const queueDir = path.join(os.tmpdir(), 'zenithjoy-agent-toutiao');
  ...
  console.log(`[handler:toutiao] spawning ${SCRIPT_PATH} taskId=${taskId}`);
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      console.log(`[handler:toutiao] task ${taskId} exit=${code}`);
      try {

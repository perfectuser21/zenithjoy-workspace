// services/agent/src/handlers/zhihu-publish.ts
//
// 真调 zhihu-publisher 走知乎写文章页 dry-run，但 *不上传图片、不点发布按钮*。
//
// 安全约束（v0.3）：
//   只 spawn `publish-zhihu-image-dryrun.cjs`（仅验证已登录 + 拦截发布 API，
//   绝不调 /api/v4/articles 等真发 API，绝不污染知乎公域）。
//   不要切回真发脚本 publish-zhihu-article.cjs。
//
// 输入 payload.content：
...
const SCRIPT_PATH = resolveScriptPath(
  'publishers',
  'zhihu-publisher',
  'publish-zhihu-image-dryrun.cjs',
);
...
function ensureSampleImage(): string {
  const dir = path.join(os.tmpdir(), 'zenithjoy-agent-zhihu');
...
export async function handleZhihuPublish(
  taskId: string,
  ...
  const queueDir = path.join(os.tmpdir(), 'zenithjoy-agent-zhihu');
  ...
  console.log(`[handler:zhihu] spawning ${SCRIPT_PATH} taskId=${taskId}`);
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      console.log(`[handler:zhihu] task ${taskId} exit=${code}`);
      try {

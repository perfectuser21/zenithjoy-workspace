/**
 * Sprint 07282119-agent-panel-knife2-android — Golden Path Step 6 回归测试。
 *
 * PanelLine02Bridge 桥接中台 line02(安卓) panel-active-tasks 数据 → 本地 PanelEventBus，
 * 真实调用 PanelEventBus.ingest()（真 bus，不 mock），只 mock 桥接模块对外发起的 fetch()
 * 这一层"更外层的无关依赖"（跨主机网络调用中台 API 是合法外部边界）。
 *
 * 与 sprints/07282119-agent-panel-knife2-android/tests/panel-line02-bridge.test.ts
 * 同款断言——那份是合同 Red 阶段锁定证据，这份是落进真实仓库测试树的永久回归覆盖
 * （DoD [BEHAVIOR] 用 `cd services/agent && npx vitest run
 * src/shared/__tests__/panel-line02-bridge.test.ts` 校验，必须能在真实路径找到）。
 */
import {
  describe, it, expect, vi, beforeEach, afterEach,
} from 'vitest';
import { PanelEventBus } from '../panel-event-bus';
import { PanelLine02Bridge } from '../panel-line02-bridge';

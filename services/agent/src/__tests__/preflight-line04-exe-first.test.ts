...
// 且锁 4.1.8.x（2026-06-29 反转 6-24 放开上界）：>=4.1.9（含 4.1.10）不支持 → 触发降级到 4.1.8。
//
// exe-first 的价值正在此：WeChat 4.1.8.107 启动后几秒会把注册表写成 4.1.10.27（可用更新版本），
// 注册表会误判成 4.1.10（不支持）→ autoRepair 误降级；exe FileVersion 始终是真实安装版本 4.1.8.107，
// 优先读 exe 即可绕开这个误判（不会把正常的 4.1.8 当 4.1.10 反复卸装）。
...
describe('preflight.js checkWechatVersion — exe-first 回归测试', () => {
  ...
  it('MOCK_WECHAT_VERSION=4.1.10.27 → ok:false（锁 4.1.8.x，>=4.1.9 触发降级）', () => {
  ...
  it('isWechatVersionSupported: 只认 4.1.8.x（4.1.10/5.x → false，< 4.1.8 → false）', () => {
    const preflight = require(PREFLIGHT_PATH);
    ...
    expect(preflight.isWechatVersionSupported('4.1.9.57')).toBe(false);
    expect(preflight.isWechatVersionSupported('5.0.0.0')).toBe(false);
  ...
  it('parseWechatVersionFromRegOutput: 4.1.10.27 解析正确 → isWechatVersionSupported = false（锁 4.1.8.x）', () => {
    const preflight = require(PREFLIGHT_PATH);
    // 注册表（被微信自动改写为可用更新版本）解析出 4.1.10.27 → 不支持（锁 4.1.8.x，触发降级）
    const v = preflight.parseWechatVersionFromRegOutput(

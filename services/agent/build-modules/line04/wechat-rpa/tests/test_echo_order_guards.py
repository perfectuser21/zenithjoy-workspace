...
def test_persistence_paths_isolated_from_real_files():
    # 部分测试会 del sys.modules 重导入 listen_chat → 用函数内 import 拿当前副本
    import listen_chat as _lc
    assert "zj-test-state-" in _lc._SENT_TEXTS_FILE, (
        f"conftest 必须经 ZJ_STATE_DIR 把持久化路径重定向到 tmp（实际 "
        f"{_lc._SENT_TEXTS_FILE!r}）——CI 在 rog 上跑 pytest 曾把真实文件覆盖成"
        "测试垃圾（2026-07-03 事故）"
    )
    assert "zj-test-state-" in _lc._REPLY_ANCHOR_FILE
    assert "zj-test-state-" in _lc._REPLIED_FILE

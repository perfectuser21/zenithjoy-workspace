...
async function main(queueFilePath) {
  ...
  // page.route 安全栅栏 — 拦截视频发布 API，命中即 dry-run 失守
  let forbiddenApiHit = null;
  await page.route(/\/rest\/cp\/works\//, async (route) => {
    const method = route.request().method();
    const url = route.request().url();
    if (method === 'POST' || method === 'PUT') {
      forbiddenApiHit = `${method} ${url}`;
      _log('[KS-VID-DRY] !! 检测到疑似视频发布 API !!', forbiddenApiHit);
      await route.abort();
      return;
    }
  ...
  try {
    _log('[KS-VID-DRY] Step 1: 导航到视频发布页');

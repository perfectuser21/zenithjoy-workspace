#!/usr/bin/env node
/**
 * 快手视频「dry-run」自检脚本（draft-only wrapper）
 *
 ...
 *
 * 安全约束：
 *   - **绝不上传视频、绝不填表单、绝不点击发布按钮**
 *   - page.route 拦截疑似发布 API，命中即 dry-run 失守 → exit 1
 *
 * 用法：
 *   node publish-kuaishou-video-dryrun.cjs <queue-file-path>
 *
 * 输出：
 *   stdout 最后一行为 JSON：
 *     {"ok":true,"dryRun":true,"url":"<最终页面 URL>","title":"<标题>"}
 *   失败时 exit code 非 0。
 */
...
async function buildContext() {
  ...
  _log('[KS-VID-DRY] 模式: CDP connectOverCDP', CDP_URL);
...
async function main(queueFilePath) {
  ...
  _log('[KS-VID-DRY] 已获取 page');

  // page.route 安全栅栏 — 拦截发布 API，命中即 dry-run 失守
  let forbiddenApiHit = null;
  await page.route(/\/(rest\/cp\/works\/|rest\/cp\/video\/publish)/, async (route) => {
    const method = route.request().method();
    const url = route.request().url();
    if (method === 'POST' || method === 'PUT') {
      forbiddenApiHit = `${method} ${url}`;
      _log('[KS-VID-DRY] !! 检测到疑似发布 API !!', forbiddenApiHit);
      await route.abort();
      return;
    }
  ...
  try {
    _log('[KS-VID-DRY] Step 1: 导航到发布视频页');
    ...
    _log('[KS-VID-DRY] Step 2: 验证页面已就位（不操作 DOM）');
    await page.waitForTimeout(2000);
    const finalUrl = page.url();
    _log('[KS-VID-DRY] 当前 URL:', finalUrl);
...
if (!queueFilePath) {
  console.error('用法: node publish-kuaishou-video-dryrun.cjs <queue-file-path>');
  process.exit(1);
}

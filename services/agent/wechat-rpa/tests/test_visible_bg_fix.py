...
def test_ensure_tray_visible_visible_returns_visible():
    ...
    def _side_effect(h, rect_byref):
        ...
        rect_byref._obj.right = 900
        rect_byref._obj.bottom = 800
        return 1

    user32.GetWindowRect.side_effect = _side_effect
...
def test_ensure_tray_visible_visible_moves_to_offscreen():
    ...
    def _side_effect(h, rect_byref):
        ...
        rect_byref._obj.right = 1100
        rect_byref._obj.bottom = 700
        return 1
    ...
    assert call_args[1] == -2600, f"SetWindowPos x 必须是 -2600，实际是 {call_args[1]}"
    assert call_args[2] == 60, f"SetWindowPos y 必须是 60，实际是 {call_args[2]}"
...
def test_ensure_tray_visible_visible_saves_original_pos():
    ...
    def _side_effect(h, rect_byref):
        ...
        rect_byref._obj.right = 950
        rect_byref._obj.bottom = 850
        return 1
...
def test_ensure_tray_visible_visible_already_offscreen_noop():
    ...
    def _side_effect(h, rect_byref):
        rect_byref._obj.left = -2600
        rect_byref._obj.top = 60
        rect_byref._obj.right = -1800
        rect_byref._obj.bottom = 660
        return 1
...
def test_restore_window_state_visible_moves_back():
    ...
    user32.SetWindowPos.assert_called()
    call_args = user32.SetWindowPos.call_args[0]
    assert call_args[1] == 300, f"还原 x 必须是 300，实际是 {call_args[1]}"
    assert call_args[2] == 100, f"还原 y 必须是 100，实际是 {call_args[2]}"
...
def test_version_is_1030():
    """manifest.json 版本号必须是 1.0.30（v1.0.30 visible-bg-fix）。"""
    import json
    candidates = [
    ...
    assert version == "1.0.30", f"manifest.json version 必须是 '1.0.30'，实际是 {version!r}"

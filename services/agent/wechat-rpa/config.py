...
# ── 消息去重与冷却 ────────────────────────────────────────
SENDER_COOLDOWN_SECONDS: float = 2.0
"""成功回复后同一发件人的冷却时间（秒）
   30→15→2（2026-07-03 用户决策：响应速度优先）：只防同一瞬间重复触发，
   不再压响应——连发按到达节奏逐条/小批回（DeepSeek 3.2 时代的体感）。
   重复回复由 replied 去重 + DELIVERED 提交双闸兜住，与冷却无关。"""

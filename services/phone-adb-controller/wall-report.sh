...
do_complete() { # task_id completed | task_id failed error_code failed_step
  local b
  if [ "$2" = "failed" ]; then
    b=$(python3 -c 'import json,sys;print(json.dumps({"outcome":"failed","executor_id":sys.argv[1],"error_code":sys.argv[2],"failed_step":int(sys.argv[3] or 0)}))' "$EXECUTOR" "$3" "${4:-0}")
  else
